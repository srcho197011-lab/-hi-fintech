...
for sk_, sl_, sb_ in STREAMS:
    for i in range(5):
        a_, b_ = ycols(i)
        cell(Y, f"{YC[i]}{YR['eff_'+sk_]}", f"=IF(AND({i}>0,NOT({REVMODE}=2)),1,SUM('월별예산'!{a_}{MR['g_'+sk_]}:{b_}{MR['g_'+sk_]}))", BLACK, F_PCT)
...
for t_ in [
    ...
    "9. 사용료 모수(공급 건수)가 문서마다 다르다. 모델은 누적 동의에 매년 60%(2031 378만 건 연 환산), IM p7은 연 신규 동의×60%(2031 150만 건). 검증보고서 v1.1의 「동일 산식」은 틀렸다. 2027은 같지만 2028~2031이 크게 다르다 — 대표 결정 필요. v3.0은 여기에 개시 일정·월별 누적을 곱해 2027 3.8만 · 2031 309만 건을 반영한다.",
