...
"""마크다운 설계문서 → 인쇄 가능한 HTML (하이핀 문서 표준)
   사용: python scripts/md2doc.py <in.md> <out.html> [문서제목]
    ...
    while i < len(lines):
        ...
        if re.match(r'^\s*[-*] ', ln):
            ...
            while i < len(lines) and (re.match(r'^\s*[-*] ', lines[i]) or (buf and lines[i].startswith("  ") and lines[i].strip())):
                if re.match(r'^\s*[-*] ', lines[i]): buf.append(inline(re.sub(r'^\s*[-*] ', '', lines[i])))
                else: buf[-1] += " " + inline(lines[i].strip())
                i += 1
        ...
        if re.match(r'^\s*\d+\. ', ln):
            ...
            while i < len(lines) and (re.match(r'^\s*\d+\. ', lines[i]) or (buf and lines[i].startswith("   ") and lines[i].strip())):
                if re.match(r'^\s*\d+\. ', lines[i]): buf.append(inline(re.sub(r'^\s*\d+\. ', '', lines[i])))
                else: buf[-1] += " " + inline(lines[i].strip())
                i += 1
        ...
        if buf: out.append("<p>" + "<br>".join(buf) + "</p>")
    return "\n".join(out)
