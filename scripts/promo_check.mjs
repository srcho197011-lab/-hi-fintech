...
const p = await b.newPage(); const [VW, VH] = (process.env.VP || "1280x720").split("x").map(Number); await p.setViewport({ width: VW, height: VH });
...
const ts = process.env.TS ? process.env.TS.split(",").map(Number) : [6, 20, 27, 50.5, 71, 84, 97, 108, 118.5]; const got = [];
