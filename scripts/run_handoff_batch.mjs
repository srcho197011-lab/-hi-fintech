...
/* ── ② 전원 조립 검증 — 프로 696명 전원(코드 목록은 페이지에서) ── */
const proCodes = await p.evaluate(() => window.__hifinPros());
const rc = { checked: 0, viol: [], sum: 0, max: 0, byGrade: {}, zero: 0 };
const PCH = 24;
for (let i = 0; i < proCodes.length; i += PCH) {
  const part = await p.evaluate((codes, date) => {
    ...
    for (const c of codes) {
      ...
      out.push({ code: c.code, n: r.rows.length, g, viol: v });
    }
    return out;
...
const report = { date: DATE, total: agg.n, cards: agg.cards, publishable: agg.pub, locked: agg.locked,
  ...
  pros: rc.checked, rosterViol: rc.viol.length, avgRoster: Number(avg), maxRoster: rc.max, zeroRosterPros: rc.zero,
  byRosterGrade: rc.byGrade, seconds: secs, pass };
...
writeFileSync(join(ROOT, "src/data/hmOpsSnapshot.js"),
  ...
  + "const HM_OPS_SNAPSHOT = " + JSON.stringify({ date: report.date, total: report.total, cards: report.cards, publishable: report.publishable,
    ...
    pros: report.pros, rosterChecked: report.pros, rosterViol: report.rosterViol, avgRoster: report.avgRoster, maxRoster: report.maxRoster,
    byRosterGrade: report.byRosterGrade, seconds: report.seconds, pass: report.pass }) + ";\n", "utf8");
