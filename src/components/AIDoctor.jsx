...
const GN_DEVICE_TOPICS = [
  { keys: ["주름", "리프팅", "탄력", "안티에이징", "동안피부", "피부탄력"], area: "피부 탄력·주름 관리",
    ...
    follow: ["아토피피부염", "건선", "두드러기", "🏥 병원·진료 안내"] },
  { keys: ["피부관리", "피부미용", "피부톤", "모공", "뷰티디바이스"], area: "피부 관리",
    ...
    follow: ["아토피피부염", "건선", "대상포진", "🏥 병원·진료 안내"] },
  ...
  { keys: ["여성건강", "여성질환", "여성질병", "질건강", "골반저", "케겔", "산후회복"], area: "여성 건강·골반저 케어",
    ...
    follow: ["방광염", "질염", "요실금", "자궁근종", "🏥 병원·진료 안내"] },
];
// 일반(비질병) 주제 상담 — 생활관리 정보를 먼저 주고, 아래 질문도우미로 관련 질병을 고르면 본격 상담으로 이어짐.
function gnDeviceCounsel(text) {
  ...
  items.push(hit.dev);
  const picks = (hit.follow || []).filter((x) => !/^🏥/.test(x)).slice(0, 3).join(" · ");
  return { bubbles: [
    ...
    { kind: "text", text: picks ? `혹시 아래 증상·질환이 궁금하시면 눌러 주세요 — 바로 자세히 상담해 드릴게요.\n(예: ${picks} 등)` : "궁금한 증상·질환을 눌러 주시면 자세히 상담해 드릴게요." },
