...
function AIDoctorSection({ onText, onVoice }) {
  ...
  const run = (query, f) => {
    ...
    if (member && /내 건강|건강상태|분석해|조심|가장.*암|의료비|보험|건강지갑|보험료|줄일|필요한|내가/.test(qq)) {
    ...
    <div className={`aidoc ${big ? "big" : ""}`}>
      ...
      {!submitted && (<>
        {(typeof demoCurrentUser === "function" && demoCurrentUser()) && (<>
          <div className="aidrech">⭐ {demoCurrentUser().name}님 맞춤 질문 <span className="clickhint"><Info size={11} /> 데모 로그인됨</span></div>
          <div className="aidfav">{["내 건강상태를 분석해줘", "내가 가장 조심해야 할 암은?", "내 의료비가 얼마나 늘어날까?", "나에게 필요한 보험은?", "건강지갑으로 보험료를 얼마나 줄일 수 있어?"].map((qq) => <button key={qq} onClick={() => run(qq)}>{qq}</button>)}</div>
        </>)}
        {FAVORITES.length > 0 && (
...
function aiRespond(text, corpus, report, QA) {
  ...
  if (has("보험", "청구", "보험금", "보장"))
    return { bubbles: [
      { kind: "text", text: "지갑에 연동된 보험 약관을 기준으로 예상 보험금을 계산해 드려요.\n(실제 지급은 보험사 심사 결과에 따라요 — 예상 금액 안내예요.)" },
      { kind: "card", card: { title: "보험 보장 도우미", items: ["가입 보험 보장 항목 조회", "검사·치료별 예상 보험금", "서류 없는 스마트 청구", "정식 보험상담 연결(설계사/GA)"], buttons: ["예상 보험금 계산", "청구하기"] } },
    ], quicks: ["예상 보험금 계산", "청구하는 법", "무상 검진보험이 뭔가요?"] };
  if (has("퇴원", "재가", "수술", "간병", "돌봄", "방문간호", "재활"))
    return { bubbles: [{ kind: "text", text: "퇴원 후 관리가 궁금하시군요. 보통 방문간호·재활·식단관리·원격 모니터링을 함께 설계해요. 필요한 항목을 알려주시면 재가/돌봄서비스를 매칭해 드릴게요." }], quicks: ["재가/돌봄서비스 신청", "간병보험 청구 가능?", "방문재활 알아보기"] };
  if (has("식단", "영양", "음식"))
  ...
  if (has("안내", "어떻게", "무엇을", "뭘 물", "뭐 물", "뭐라고", "도와", "도움", "기능", "사용법", "메뉴", "할 수 있", "할수있", "예시", "물어보면", "물어볼"))
    return { bubbles: [
      ...
      { kind: "card", card: { title: "AI 주치의 이용 안내", items: ["내 건강리포트 요약·질병/암 위험도", "질환별 증상·검사·치료·생활습관(질병관리청 Q&A 1,947쌍)", "의료비·생체나이 예측", "보험 보장조회·청구 안내"], buttons: ["내 리포트 요약", "갑상선염의 증상은 무엇인가요?", "의료비 예측"] } },
      { kind: "text", text: "예: ‘고혈압 생활습관 관리법은?’, ‘위암 검사는 어떻게 하나요?’, ‘내 당뇨 위험’처럼 물어보세요." },
    ], quicks: ["내 리포트 요약", "당뇨 검사 방법", "고혈압 생활습관 관리법은?", "보험금 청구"] };
  return { bubbles: [{ kind: "text", text: `‘${text}’에 대한 정보를 찾지 못했어요. 😅 질환 이름과 함께 ‘증상·검사·치료·생활습관’을 물어보시거나, 아래 추천 질문을 눌러보세요. 리포트 분석·의료비·보험도 도와드려요.` }], quicks: ["내 리포트 요약", "당뇨 검사 방법", "갑상선염의 증상은 무엇인가요?", "의료비 예측", "보험금 청구"] };
}
...
function Chat() {
  ...
  return (
    <div className="kt">
      ...
      <div className="kt-disc">AI 상담은 의료진의 진단을 대체하지 않으며, 보험금은 예상 안내입니다.</div>
    </div>
  );
}
