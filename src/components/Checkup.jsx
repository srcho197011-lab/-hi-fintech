...
const CI_CONSENT = {
  ...
  to: "현대해상(인수사) · 한국신용정보원 · 보험협회 · 보험료 산출 기관 · 재보험사(국외 포함) · 금융결제원 등 법령상 기관",
  keep: "계약이 끝난 뒤 5년까지(법에서 정한 기간)",
  groups: [
    { g: "use", t: "① 가입 심사를 위해 확인합니다", why: "보험에 가입할 수 있는지, 어디까지 보장되는지 정하기 위해 필요해요.",
      note: "아래 인적·보험계약 정보는 계약에 꼭 필요해 별도 선택 없이 포함돼요 — 성명·주소·생년월일·이메일·전화·성별·국적·직업·운전여부, 보험계약·보험금·금융거래 정보, 가입 전 알릴 사항, 건강검진 결과정보.",
      items: [
    ...
    { g: "give", t: "② 심사·지급에 필요한 곳에 전달합니다", why: "중복 확인·재보험·의료 심사처럼 법이 정한 업무에 쓰여요.",
      items: [
        ...
        { k: "ci_give_pi", req: true, t: "인적·금융·보험계약 정보", why: "계약 관리와 보험금 지급 업무에 쓰여요.", d: "성명·주소·생년월일·연령·이메일·전화·성별·국적·직업·운전여부 / 납입 계좌 등 금융거래 정보 / 보험계약·보험금 정보 / 가입 전 알릴 사항 / 소득 및 재산 정보" },
...
function BookingModal({ center, mode, onClose }) {
  ...
  const enrollConfirm = () => {
    if (!(enroll.termsOk && enroll.req && ciOk && idOk)) return;
    try {
            ...
            <div style={{ border: "1.5px solid " + (enroll.st === "done" ? "#16A34A" : enroll.st === "skip" ? "var(--border)" : "#F59E0B"), borderRadius: 13, padding: "12px 13px", background: enroll.st === "done" ? "#F0FDF4" : "#FFFDF7", marginTop: 10 }}>
              {enroll.st === "done" ? (
                  ...
                  {enroll.ciOpen && (
                    <details style={{ marginTop: 8, fontSize: 11 }}>
                      ...
                      <div style={{ color: "#475569", lineHeight: 1.62, marginTop: 6 }}>
                        <b>1. 수집·이용</b> — 목적: 보험계약의 인수심사·체결·이행·유지·관리, 보험료 산출·검증, 민원처리 및 분쟁 대응, 금융거래 업무, 보험모집질서 유지, 가입한 보험계약 상담, 만족도 조사, 보험금 지급·심사, 모집인 수당 확인, 건강등급 산정, 기존 계약과의 중요사항 비교설명. 보유·이용기간: 동의일부터 거래 종료 후 5년(거래 종료일 = 만기·해지·취소·철회·소멸일, 보험금 청구권 소멸시효 완성일, 채권·채무 소멸일 중 가장 나중에 도래한 날).<br /><br />
                        <b>2. 제공</b> — 제공받는 자: 금융위원회·금융감독원·국세청·보험요율산출기관 등 법정 업무수행기관, 한국신용정보원, 생명·손해보험회사 및 국내 재보험사·공제사업자·체신관서, 계좌개설 금융기관·금융결제원·은행연합회, 생명·손해보험협회, 국외 재보험사. 제공 목적: 법령상 업무 수행, 신용정보 집중관리·활용, 중복보험 확인 및 비례보상, 재보험 가입, 보험계약 공동인수, 의료심사·자문, 사고처리·민원접수 등. 보유기간: 제공받는 자의 이용목적 달성 시까지(관련 법령상 보존기간에 따름). ※ 외국 재보험사의 국내지점이 재보험 가입판단·공동인수 지원업무를 위탁하기 위한 경우 별도 동의 없이 외국 소재 본점에 정보가 이전될 수 있습니다. ※ 업무위탁 목적으로 처리하는 경우 별도 동의 없이 수탁자에게 제공될 수 있습니다.<br /><br />
                        <b>3. 조회</b> — 조회 대상: 한국신용정보원, 생명·손해보험협회, 보험요율산출기관 등. 조회 목적: 보험계약의 인수심사·체결·이행·유지·관리(부활 및 갱신 포함), 가입한도 조회, 실손형 보험 중복 확인, 기존 계약과의 중요사항 비교설명, 세금우대저축사항 확인, 보험금 지급·심사, 보험사고 조사. 조회 결과 보험거래가 개시되면 거래 종료 후 5년까지 동의 효력이 지속되며, 거래가 거절된 경우 그 시점부터 효력이 소멸합니다.<br /><br />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 8, marginTop: 9 }}>
                  <button className="cbtn pri" style={{ margin: 0, minWidth: 0, opacity: enroll.termsOk && enroll.req && ciOk && idOk ? 1 : .5 }} disabled={!(enroll.termsOk && enroll.req && ciOk && idOk)} onClick={enrollConfirm}><ShieldCheck size={14} /> {!ciOk ? "개인(신용)정보 동의가 필요해요" : (!idOk ? "계약자 확인 후 가입돼요" : "가입 확정 · 보험료 0원")}</button>
