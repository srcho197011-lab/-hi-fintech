...
function BookingModal({ center, mode, onClose }) {
  ...
  const [enroll, setEnroll] = useState(() => ({ st: "idle", termsOpen: false, termsOk: false, req: false, mkt: false, name: (typeof demoCurrentUser === "function" && demoCurrentUser() && demoCurrentUser().name) || (typeof authCurrent === "function" && authCurrent() && authCurrent().name) || "", rrn1: "", rrn2: "", ci: {}, ciOpen: false, ciDetail: {}, ciFull: false }));
  ...
  const enrollConfirm = () => {
    if (!(enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk)) return;
    try {
      ...
      if (m && typeof vaultSaveConsents === "function") {
        ...
        vaultSaveConsents(m, Object.assign({ ins_terms: true, ins_enroll: true, ins_third: true, mkt: !!enroll.mkt,
          ...
          insuredName: enroll.name.trim(), rrnMasked, rrnHash }, ciState));
      }
            ...
            <div style={{ border: "1.5px solid " + (enroll.st === "done" ? "#16A34A" : enroll.st === "skip" ? "var(--border)" : "#F59E0B"), borderRadius: 13, padding: "12px 13px", background: enroll.st === "done" ? "#F0FDF4" : "#FFFDF7", marginTop: 10 }}>
              {enroll.st === "done" ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 8, marginTop: 9 }}>
                  <button className="cbtn pri" style={{ margin: 0, minWidth: 0, opacity: enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk ? 1 : .5 }} disabled={!(enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk)} onClick={enrollConfirm}><ShieldCheck size={14} /> {!ciOk ? "개인(신용)정보 동의가 필요해요" : (!enroll.mkt ? "상담·안내 동의가 필요해요" : (!idOk ? "계약자 확인 후 가입돼요" : "가입 확정 · 보험료 0원"))}</button>
