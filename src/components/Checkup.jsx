...
/* ── 개인(신용)정보 동의서 — 원문(신용정보동의서.docx) 체크 9건을 3그룹으로 압축 표기.
   압축은 문구·시각 층위일 뿐이며 동의 단위는 병합하지 않는다(개보법 §22② 구분 동의·§23 민감·§24 고유식별·§17 제3자, 신용정보법 §32 조회).
   고유식별정보(주민등록번호 등) 제공은 보험업법상 별도 요구 사항으로 원문 그대로 유지한다. */
const CI_CONSENT = {
  ver: "개인(신용)정보 동의서 v1.0",
  to: "현대해상(인수사) · 한국신용정보원 · 보험협회 · 보험료 산출 기관 · 재보험사 · 금융결제원 등 법령상 기관",
  keep: "계약이 끝난 뒤 3년까지",
  groups: [
    { g: "use", t: "① 가입 심사를 위해 확인합니다", why: "보험에 가입할 수 있는지, 어디까지 보장되는지 정하기 위해 필요해요.",
      note: "아래 인적·보험계약 정보는 계약에 꼭 필요해 별도 선택 없이 포함돼요 — 성명·주소·생년월일·이메일·전화·성별, 보험계약·보험금·금융거래 정보, 가입 전 알릴 사항, 건강검진 결과정보.",
      items: [
        { k: "ci_use_uid", req: true, t: "주민등록번호 등 나를 특정하는 번호", why: "보험업법에 따라 계약자·피보험자가 본인인지 확인해요.", d: "주민등록번호·외국인등록번호·여권번호·운전면허번호" },
        { k: "ci_use_sens", req: true, t: "질병·상해 등 건강에 관한 정보", why: "보장 대상인지 판단하기 위해 확인해요.", d: "상병명·진료기록·기왕증 등 피보험자의 질병·상해 정보, 한국신용정보원이 보유한 보험사고·질병 정보" },
      ] },
    { g: "give", t: "② 심사·지급에 필요한 곳에 전달합니다", why: "중복 확인·재보험·의료 심사처럼 법이 정한 업무에 쓰여요.",
      items: [
        { k: "ci_give_uid", req: true, t: "주민등록번호 등 나를 특정하는 번호", why: "받는 기관에서도 동일인 확인이 필요해요.", d: "주민등록번호·외국인등록번호·여권번호·운전면허번호" },
        { k: "ci_give_sens", req: true, t: "질병·상해 등 건강에 관한 정보", why: "보험금 심사와 중복보험 확인에 쓰여요.", d: "피보험자의 질병·상해에 관한 정보(상병명·진료기록·기왕증 등)" },
        { k: "ci_give_pi", req: true, t: "인적·금융·보험계약 정보", why: "계약 관리와 보험금 지급 업무에 쓰여요.", d: "성명·주소·생년월일·연령·이메일·전화·성별 / 납입 계좌 등 금융거래 정보 / 보험계약·보험금 정보 / 가입 전 알릴 사항" },
        { k: "ci_give_extra", req: false, t: "[선택] 성별·연령·보험계약 종류 추가 제공", why: "상품 개발·통계에 쓰여요.", gain: "통계 개선으로 더 맞는 안내를 받아요.", loss: "맞춤 정확도가 조금 낮아져요 (가입·보장은 그대로예요).", d: "성별, 연령, 보험계약정보(상품종류·기간)" },
      ] },
    { g: "inq", t: "③ 이미 든 보험을 확인합니다", why: "겹치는 보험이 있는지 확인해 불필요한 가입을 막아드려요.",
      items: [
        { k: "ci_inq_uid", req: true, t: "주민등록번호 등 나를 특정하는 번호", why: "조회 기관에서 본인 계약을 찾기 위해 필요해요.", d: "주민등록번호·외국인등록번호·여권번호·운전면허번호" },
        { k: "ci_inq_sens", req: true, t: "질병·상해 등 건강에 관한 정보", why: "보험금 심사·보험사고 조사에 쓰여요.", d: "피보험자의 질병·상해에 관한 정보(상병명·진료기록 등)" },
        { k: "ci_inq_pi", req: true, t: "보험계약·보험금 정보", why: "실손보험 중복 여부와 가입 한도를 확인해요.", d: "성명·국내거소신고번호, 세금우대저축 관련 정보, 보험계약정보(상품종류·기간·보험료·가입금액), 보험금 정보" },
      ] },
    /* ④ S4 건강관리 서비스(선택) — 목적 문언은 부수업무 신고 범위와 동일 표현(형 붙임 정렬 원칙).
       ⚠️ S2(인수·지급 심사)와 목적이 다른 별개의 처리 — 저장·이용 경로 분리(Part IV 핵심) */
    { g: "care", t: "④ [선택] 건강 유지·증진 및 질병의 사전예방·악화 방지를 위한 건강관리 서비스에 씁니다",
      why: "담당 프로(헬스케어 전문가)가 검진 결과 해설·건강 코칭·추적 관리를 해드리기 위한 동의예요. 동의하지 않으셔도 검진 예약·보험 가입에는 아무 불이익이 없어요.",
      note: "근거: 신용정보법 제33조 제2항·같은 법 시행령 제28조의2, 보험업법 제11조의2(부수업무). 이 동의로 쓰는 정보는 가입 심사·보험금 지급 목적과 분리되어 별도로 관리돼요 — 심사에 쓰이지 않아요.",
      items: [
        { k: "ci_care_svc", req: false, t: "[선택] 건강관리 서비스 제공(결과 해설·코칭·추적 관리)", why: "동의는 언제든 철회할 수 있어요.", gain: "담당 헬스케어 전문가가 결과 해설·코칭·추적 관리를 평생 무료로 해드려요.", loss: "담당 전문가가 배정되지 않아요 — 결과 해설 전화·맞춤 케어 키트·코칭을 받을 수 없어요. (검진·가입에는 영향 없어요)", d: "건강검진 결과의 등급·관리필요 항목(원본 수치 아님), 건강관리 활동 기록(진료 안내·미션·기기 기록), 연락 가능 시간대" },
      ] },
    /* ⑤ N1 만기 보장분석(선택 — 리뉴얼 v1.1 R1, 형 승인 2026-09-02) — v2 §4-3 신설 동의.
       무인 분석(사람 관여 없음)·보장맵만 산출·건강정보 미사용 — 효력은 보장 만기 후 3개월(권고안·형 확정란) */
    { g: "matan", t: "⑤ [선택] 보장이 끝나기 전에 내 보장 상태를 분석해 드립니다",
      why: "무료 보장이 끝나기 20일 전에, 시스템이 자동으로 지금 갖고 계신 보장을 분석해서 「어디가 비어 있고 어디가 겹치는지」 보장맵을 만들어 드려요. 사람이 들여다보는 게 아니라 서버가 계산하고, 건강정보는 쓰이지 않아요 — 계약 정보만으로 계산돼요.",
      note: "분석 전에 한 번 더 알려드리고, 원하지 않으시면 그때 거부하실 수 있어요. 이 동의의 효력은 보장 만기 후 3개월까지예요. 동의하지 않으셔도 검진·보장 이용에는 아무 불이익이 없어요.",
      items: [
    ...
    { g: "video", t: "⑦ [선택] 담당 전문가와 영상으로 상담하시겠어요?",
      ...
      note: "정해진 시간대는 없어요 — 회원님이 편하신 때가 상담 시간이에요. 먼저 여쭤보고 좋다고 하실 때만 연결돼요. 카메라는 회원님이 켜고 끄시고, 통화 중 언제든 음성이나 문자로 바꾸실 수 있어요. 영상과 음성은 저장하지 않아요 — 통화가 끝나면 상담 요약만 남고, 그 요약도 회원님이 확인하신 뒤에 저장돼요. 동의하지 않으셔도 전화·문자로 똑같이 안내드려요.",
      items: [
    ...
    { g: "ads", t: "⑥ [선택] 유용한 소식을 보내드려도 될까요 — 받고 싶은 방법만 골라 주세요",
      ...
      note: "밤 9시~아침 8시에는 보내지 않아요(야간 전송은 별도 동의가 있어야 해요). 동의하지 않으셔도 예약 확인·결과 도착 같은 꼭 필요한 알림은 정상적으로 와요.",
      items: [
        { k: "ci_ads_push", req: false, t: "[선택] 앱 알림으로 받기", why: "가장 빠르고 조용한 방법이에요.", gain: "결과·혜택 소식을 가장 빠르게 받아요.", loss: "유용한 소식을 놓칠 수 있어요 (예약·결과 등 필수 알림은 그대로 와요).", d: "광고성 앱 푸시 알림" },
        { k: "ci_ads_sms", req: false, t: "[선택] 문자로 받기", why: "앱을 자주 안 여시면 문자가 편해요.", gain: "앱을 안 열어도 중요한 혜택을 문자로 받아요.", loss: "앱 미접속 시 혜택 소식을 놓쳐요.", d: "광고성 문자(SMS·알림톡)" },
        { k: "ci_ads_email", req: false, t: "[선택] 이메일로 받기", why: "자세한 소식은 이메일이 좋아요.", gain: "자세한 건강 리포트 소식을 이메일로 받아요.", loss: "상세 안내 자료를 받지 못해요.", d: "광고성 이메일" },
      ] },
  ],
};
const CI_ALL = CI_CONSENT.groups.reduce((a, g) => a.concat(g.items), []);
const CI_REQ = CI_ALL.filter((i) => i.req).map((i) => i.k);

/* 다른 섹션(AI 주치의 등)에서 특정 검진 탭으로 진입하기 위한 인텐트 */
let _checkupTab = null;
let _checkupGo = null;   // 모달 등 외부에서 탭 전환(원격상담 동선) — CheckupSection 마운트 시 연결
...
/* 탭 별칭 — 구 딥링크(nat·personal)를 통합 탭(health)으로 연결(깨진 진입 0) */
function _checkupTabAlias(k) { return (k === "nat" || k === "personal") ? "health" : k; }
function CheckupSection() {
  const [cat, setCatRaw] = useState(_checkupTabAlias(_checkupTab) || "health");
  /* 통합 탭 내부의 검진 유형 필터 — 구 personal 딥링크는 개인종합 모드로 유지 */
  const [healthMode, setHealthMode] = useState(_checkupTab === "personal" ? "personal" : "all");
  const setCat = (k) => setCatRaw(_checkupTabAlias(k));
  useEffect(() => { _checkupTab = null; _checkupGo = (k, m) => { setCatRaw(_checkupTabAlias(k)); if (m) setHealthMode(m); }; return () => { _checkupGo = null; }; }, []);
  const cats = [["health", "개인건강검진", ShieldCheck], ["biz", "기업검진", Users], ["special", "특수검진", AlertTriangle], ["public", "공공검진지원", HeartHandshake], ["rec", "AI 맞춤추천", Sparkles], ["tele", "원격진료·건강상담", Video], ["doctor", "🩺 AI 주치의", Bot], ["emergency", "🚨 응급신호", AlertTriangle]];
  const HM = [["all", "전체"], ["nat", "국가검진 · 본인부담 0원"], ["personal", "개인종합검진"]];
  return (
    <div style={{ marginTop: 16 }}>
      <div className="aihead"><span className="aiico"><SecIcon k="checkup" /></span>
        <div><div style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-.5px" }}>건강검진</div><div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 2 }}>개인건강검진 · 기업검진 · 특수검진 · 공공검진지원 — 예약부터 검진 후 원격진료·건강상담까지</div></div></div>
      ...
      {cat === "health" && (<>
        {/* 검진 이후 상담 안내 스트립 — 검진→결과→원격상담 동선 */}
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 11, padding: "9px 13px", margin: "2px 0 10px", fontSize: 12.3 }}>
          <Stethoscope size={15} color="#1D4ED8" />
          <span style={{ flex: 1 }}>검진 후에는 결과를 아는 <b>전문의 원격 상담</b>까지 이어져요.</span>
          <button style={{ border: "1px solid #1D4ED8", background: "#fff", color: "#1D4ED8", fontWeight: 800, borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontSize: 11.5, flexShrink: 0 }} onClick={() => setCat("tele")}>원격진료·건강상담 ›</button>
        </div>
        <div className="bklbl" style={{ margin: "0 0 8px" }}>검진 유형</div>
        <div className="regions" style={{ marginBottom: 4 }}>{HM.map(([k, t]) => <div key={k} className={`fsel ${healthMode === k ? "on" : ""}`} onClick={() => setHealthMode(k)}>{t}</div>)}</div>
        {healthMode === "all" && <><BrandDirectory catFilter="all" /><NationalOrgDirectory /></>}
        {healthMode === "nat" && <><BrandDirectory catFilter="nat" /><NationalOrgDirectory /></>}
        {healthMode === "personal" && <><BrandDirectory catFilter="personal" /><NationalOrgDirectory personalOnly /></>}
      </>)}
      ...
      {cat === "result" && (<>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 11, padding: "9px 13px", margin: "8px 0 10px", fontSize: 12.3 }}>
          <Stethoscope size={15} color="#1D4ED8" />
          <span style={{ flex: 1 }}>결과에 궁금한 점이 있나요? <b>결과 상담하기</b> — 우리 동네 전문의 원격 상담으로 이어져요.</span>
          <button style={{ border: "1px solid #1D4ED8", background: "#fff", color: "#1D4ED8", fontWeight: 800, borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontSize: 11.5, flexShrink: 0 }} onClick={() => setCat("tele")}>결과 상담하기 ›</button>
        </div>
        <CheckupResults />
      </>)}
    </div>
...
function BrandDirectory({ only, catFilter }) {
  ...
  const natOrgs = useCheckupOrgs();   // 전국 지정기관 총수(배너 표기용 — 하드코딩 금지)
  ...
  const base = ALL.filter((c) => only ? c.b === only : catFilter === "all" ? true : catFilter ? checkupCats(c).includes(catFilter) : !FEATURED_BRANDS.includes(c.b));
    ...
    <>
      {catFilter && CHECKUP_CAT_META[catFilter] && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, background: CHECKUP_CAT_META[catFilter].color + "12", border: "1px solid " + CHECKUP_CAT_META[catFilter].color + "33", borderRadius: 14, padding: "13px 16px", marginBottom: 12 }}>
          ...
          <div><div style={{ fontSize: 15, fontWeight: 900, color: "#12203F" }}>{catFilter === "nat" && natOrgs.data
            ? <>국가검진 · 전국 지정기관 <b style={{ color: "var(--blue)" }}>{natOrgs.data.meta.total.toLocaleString()}</b>곳 <span style={{ fontWeight: 700, color: "var(--muted)", fontSize: 12.5 }}>(공단 공개 데이터 기준 · 브랜드 큐레이션 {base.length}곳)</span></>
            : <>{CHECKUP_CAT_META[catFilter].label} · 브랜드 검진기관 {base.length}곳</>}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2, lineHeight: 1.5 }}>{CHECKUP_CAT_META[catFilter].intro}</div></div>
        </div>
      )}
      {isKahp && (
...
function NationalOrgDirectory({ onlyType, personalOnly }) {
  ...
  const list = React.useMemo(() => {
    ...
    if (sgg !== "전체") L = L.filter((o) => o[2] === sgg);
    const qq = q.trim();
  ...
  const geoPts = list.filter((o) => o[6] && o[7]).slice(0, 200).map((o) => ({
    name: o[0], addr: o[4], tel: o[5] || "", tag: d.types[o[3]] + (o[8] === 1 ? " · 대표위치" : ""), lat: o[6], lng: o[7],
    _bk: { kind: "org", org: o },
  }));
  const toCenter = (o) => ({ name: o[0], r: o[1], area: o[2], ad: o[4], tags: [d.types[o[3]], "공단 지정기관"], _noPrice: true, _plan: "basic" });
  return (
    <div style={{ marginTop: 18 }}>
      ...
      {sido !== "전체" && sggs.length > 1 && (<>
        <div className="bklbl" style={{ margin: "10px 0 8px" }}>시·군·구</div>
        <div className="regions">{sggs.map((s) => <div key={s} className={`fsel ${sgg === s ? "on" : ""}`} onClick={() => { setSgg(s); reset(); }}>{s}</div>)}</div>
      </>)}
      <div className="filterbar"><div className="fsearch"><Search size={15} /><input value={q} onChange={(e) => { setQ(e.target.value); reset(); }} placeholder="전국 지정기관 검색 — 기관명·지역·주소" /></div></div>
      <MapCard open={mapOpen} onToggle={() => setMapOpen((v) => !v)} title={`지정기관 위치 지도 (필터 결과 ${Math.min(list.length, 200).toLocaleString()}곳 표시 · 좌표 심평원 매칭 ${d.meta.matchRate}%)`} accent="#1D4ED8" points={geoPts} />
      <div className="chcount">검색 결과 <b style={{ color: "var(--blue)" }}>{list.length.toLocaleString()}</b>곳 {list.length > shown && <span style={{ color: "var(--soft)", fontWeight: 600 }}>· {shown}곳 표시 중</span>}</div>
      {view.map((o, i) => {
        const tcol = ORG_TYPE_COLOR[d.types[o[3]]] || "#2563EB";
        return (
          <div className="center" key={i}>
            <div className="cimg" style={{ background: tcol + "12" }}><Building2 size={34} color={tcol} /></div>
            <div className="cmain">
              <div className="cname">{o[0]} <span className="cbadge" style={{ color: tcol, background: tcol + "1A" }}>{d.types[o[3]]}</span></div>
              <div className="cmeta"><span style={{ fontWeight: 800, color: "#2563EB" }}>{o[1]}</span>{o[2] && <> · <MapPin size={12} />{o[2]}</>}{o[5] && <> · <Phone size={12} />{o[5]}</>}</div>
              <div className="ctags"><span>국가검진(지정)</span>{o[8] === 1 && <span style={{ color: "#B45309", background: "#FEF3C7" }}>지도 대표위치</span>}</div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4, lineHeight: 1.45 }}>{o[4]}</div>
            </div>
            <div className="cright">
              <span className="cbadge"><ShieldCheck size={10} /> 무료 검진보험</span>
              <div className="obtns"><button onClick={() => openConsult("건강검진 상담 — " + o[0])}>상담</button><button className="book" onClick={() => setSel(o)}>예약</button></div>
            </div>
          </div>
        );
      })}
      {shown < list.length && <button className="cbtn" onClick={() => setShown((x) => x + 20)}>더 보기 ({(list.length - shown).toLocaleString()}곳 더)</button>}
      <div className="chnote">※ 출처: 국민건강보험공단 검진기관기본({d.meta.asOf} · 공개 파일 전체 {d.meta.nhis.toLocaleString()}행) · 건강보험심사평가원 병원정보(좌표 매칭 {d.meta.matchRate}%) · 전국보건기관표준데이터. 지정 현황은 변동될 수 있어 예약 전 기관 확인을 권장합니다. 본 목록은 국가검진 지정 근거만 표시하며(개인·기업검진 여부는 기관 확인), 의료법(의료광고 심의·환자 유인·알선 금지)을 준수합니다.</div>
      {sel && <BookingModal center={toCenter(sel)} mode="comp" onClose={() => setSel(null)} />}
    </div>
  );
}
...
/* 보험계약 가입 서명란 — 계약자 자필서명(캔버스, 마우스·터치 공용).
   서명 이미지는 계약 성립의 증빙이므로 회원 금고·증서 스냅샷에만 남기고, 체인에는 해시만 기록한다. */
function SignPad({ value, onChange }) {
  ...
  const start = (e) => { drawing.current = true; last.current = posOf(e); };
  const move = (e) => {
    if (!drawing.current) return;
    if (e.cancelable) e.preventDefault();
    const c = cv.current, g = c.getContext("2d"), pt = posOf(e);
    g.strokeStyle = "#0F172A"; g.lineWidth = 2.4; g.lineCap = "round"; g.lineJoin = "round";
    g.beginPath(); g.moveTo(last.current.x, last.current.y); g.lineTo(pt.x, pt.y); g.stroke();
    last.current = pt;
  };
  const end = () => { if (!drawing.current) return; drawing.current = false; try { onChange(cv.current.toDataURL("image/png")); } catch (err) {} };
  const clear = () => { const c = cv.current; if (c) c.getContext("2d").clearRect(0, 0, c.width, c.height); onChange(""); };
  return (
    <div>
      <div style={{ position: "relative", border: "1.5px dashed #94A3B8", borderRadius: 10, background: "#FCFDFF", overflow: "hidden" }}>
        <canvas ref={cv} width={620} height={140} className="signpad"
          onMouseDown={start} onMouseMove={move} onMouseUp={end} onMouseLeave={end}
          onTouchStart={start} onTouchMove={move} onTouchEnd={end}
          style={{ width: "100%", height: 118, display: "block", cursor: "crosshair", touchAction: "none" }} />
        {!value && <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", pointerEvents: "none", color: "#94A3B8", fontSize: 12.5, fontWeight: 600, textAlign: "center", lineHeight: 1.6 }}>이 칸에 계약자 본인이 자필로 서명해 주세요<br /><span style={{ fontSize: 11, fontWeight: 500 }}>마우스로 그리거나 휴대폰에서는 손가락으로 서명하면 돼요</span></div>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: value ? "#16A34A" : "var(--muted)" }}>{value ? "✓ 서명이 입력됐어요 — 가입증서에 그대로 표시돼요" : "서명해야 보험계약이 성립돼요"}</span>
        <button type="button" onClick={clear} style={{ border: "1px solid var(--border)", background: "#fff", borderRadius: 8, padding: "4px 11px", fontSize: 11.5, fontWeight: 700, cursor: "pointer", color: "var(--muted)" }}>지우고 다시</button>
      </div>
    </div>
  );
}
...
function BookingModal({ center, mode, onClose }) {
  ...
  const [enroll, setEnroll] = useState(() => ({ st: "idle", termsOpen: false, termsOk: false, req: false, mkt: false, name: (typeof demoCurrentUser === "function" && demoCurrentUser() && demoCurrentUser().name) || (typeof authCurrent === "function" && authCurrent() && authCurrent().name) || "", rrn1: "", rrn2: "", sign: "", ci: {}, ciOpen: false, ciDetail: {}, ciFull: false }));
  /* 계약자 확인 — 주민번호는 형식 검증 후 해시만 보관(평문 미저장·화면 마스킹). 부담 완화: 동의 완료 후에만 노출 */
  const ciOk = CI_REQ.every((k) => !!enroll.ci[k]);   // 필수 8건(선택 1건 제외) 모두 동의 시 통과
  const ciCount = CI_ALL.filter((i) => !!enroll.ci[i.k]).length;
  const rrnOk = /^\d{6}$/.test(enroll.rrn1) && /^\d{7}$/.test(enroll.rrn2);
  const idOk = (enroll.name || "").trim().length >= 2 && rrnOk;
  const signOk = !!(enroll.sign && enroll.sign.length > 120);   // 자필서명 입력 여부(빈 캔버스 dataURL 배제)
  const enrollConfirm = () => {
    if (!(enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk && signOk)) return;
    try {
      ...
      setInsAgree(true); setEnroll((s) => ({ ...s, st: "done", rrn1: "", rrn2: "" }));   // 주민번호 평문은 확정 즉시 메모리에서도 폐기(서명은 증서 표시용 유지)
  ...
  const onConfirm = (paid) => {
    ...
    if (insAgree) {
      try {
        ...
        let insured = null;
        try { const v = m && typeof vaultLoad === "function" ? vaultLoad(anonToken(m)) : null; const st = v && v.consents && v.consents.state; if (st && st.insuredName) insured = { name: st.insuredName, rrn: st.rrnMasked || "", sign: st.signImg || "", signedAt: st.signedAt || 0 }; } catch (e) {}
              ...
              {enroll.st === "done" ? (
                <div style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
                  ...
                  <div style={{ fontSize: 12.3, lineHeight: 1.55 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>일반암 진단금 기준(상피내암·갑상선암·기타피부암·경계성종양 제외) · 동의 내역은 데이터 금고에 기록{" · 상담·안내 동의 포함"} <button style={{ border: "none", background: "none", color: "#B45309", fontWeight: 700, cursor: "pointer", fontSize: 11, padding: 0, marginLeft: 4 }} onClick={() => { setInsAgree(false); setEnroll({ st: "idle", termsOpen: false, termsOk: false, req: false, mkt: false }); }}>변경</button></div>
                  </div>
                </div>
              ) : enroll.st === "skip" ? (
                ...
                {/* ②-2 개인(신용)정보 동의 — 요약 카드 1층 → 그룹·개별 체크 2층 → 전문 3층.
                    화면은 압축하되 저장은 9건 개별(구분 동의). 고유식별정보 제공은 보험업법상 별도 요구로 원문 유지. */}
                <div style={{ marginTop: 7, border: "1.5px solid " + (ciOk ? "#BBF7D0" : "#E2E8F0"), borderRadius: 11, background: ciOk ? "#F0FDF4" : "#fff", padding: "11px 12px", opacity: enroll.termsOk ? 1 : .55, pointerEvents: enroll.termsOk ? "auto" : "none" }}>
                  <div style={{ fontSize: 12.4, fontWeight: 800, display: "flex", alignItems: "center", gap: 6 }}>
                    ②-2 [필수] 개인(신용)정보 동의 {ciOk && <b style={{ color: "#16A34A" }}>동의 완료 ✓</b>}
                    <span style={{ marginLeft: "auto", fontSize: 10.5, color: "var(--soft)", fontWeight: 700 }}>{ciCount}/{CI_ALL.length}건</span>
                  </div>
                  {/* 1층 — 요약 카드: 누가·무엇을·언제까지·되돌릴 수 있나 */}
                  <div style={{ fontSize: 11.3, color: "#334155", lineHeight: 1.62, marginTop: 6 }}>
                    · <b>누가 보나요</b> — {CI_CONSENT.to}<br />
                    · <b>무엇을</b> — 성명·연락처 등 인적사항 / 보험계약·보험금 정보 / 건강에 관한 정보<br />
                    · <b>언제까지</b> — {CI_CONSENT.keep}<br />
                    · <b>되돌릴 수 있나요</b> — 네, <b style={{ color: "#16A34A" }}>언제든 철회</b>하실 수 있어요(데이터 금고 또는 하이에게 말씀만 주세요)
                  </div>
                  {/* ── 동의 UX 개편(형 승인 2026-09-02 U2): 전 항목 상시 펼침 · 최상단 전체 토글(재클릭 해제) · 손실 고지 · 재유도 팝업 없음(2024 제재 사안 방어) ── */}
                  {(() => {
                    const onCnt = CI_ALL.filter((it) => !!enroll.ci[it.k]).length;
                    const allOn = onCnt === CI_ALL.length, someOn = onCnt > 0 && !allOn;
                    const optN = CI_ALL.length - CI_REQ.length;
                    return (
                      <div style={{ marginTop: 9 }}>
                        <button className={"cbtn" + (allOn ? " pri" : "")} style={{ margin: 0, width: "100%", padding: "11px 12px", fontSize: 12.6, fontWeight: 800, display: "flex", alignItems: "center", gap: 8, justifyContent: "center", border: someOn ? "1.5px solid #1D4ED8" : undefined }}
                          onClick={() => { const c = {}; CI_ALL.forEach((it) => { c[it.k] = !allOn; }); setEnroll((st) => ({ ...st, ci: c, ciFull: true })); }}>
                          <span style={{ width: 19, height: 19, borderRadius: 5, border: "2px solid " + (allOn ? "#fff" : "#1D4ED8"), background: allOn ? "transparent" : (someOn ? "#DBEAFE" : "#fff"), display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 900, color: allOn ? "#fff" : "#1D4ED8", flexShrink: 0 }}>{allOn ? "✓" : someOn ? "－" : ""}</span>
                          모든 항목 한 번에 동의 <span style={{ fontWeight: 600, opacity: .85 }}>(필수 {CI_REQ.length} + 선택 {optN})</span>
                        </button>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 5 }}>
                          <button style={{ border: "none", background: "none", color: "#64748B", fontSize: 10.8, fontWeight: 700, cursor: "pointer", padding: 0, textDecoration: "underline" }}
                            onClick={() => { const c = {}; CI_ALL.forEach((it) => { c[it.k] = !!it.req; }); setEnroll((st) => ({ ...st, ci: c, ciFull: true })); }}>필수만 동의</button>
                          <span style={{ fontSize: 10.4, color: "var(--soft)" }}>다시 누르면 풀려요 · 항목별로도 켜고 끌 수 있어요</span>
                        </div>
                      </div>
                    );
                  })()}
                  {ciOk && (
                    <button className="cbtn" style={{ margin: "8px 0 0", padding: "6px 11px", fontSize: 11.5 }}
                      onClick={() => setEnroll((st) => ({ ...st, ciOpen: !st.ciOpen }))}>{(!ciOk || enroll.ciOpen) ? "항목 접기" : "동의한 항목 보기 · 변경"}</button>
                  )}
                  {/* 2층 — 전 그룹 상시 펼침(동의 완료 후에만 접기) · 선택 항목은 손실 고지 2줄 */}
                  {(!ciOk || enroll.ciOpen) && CI_CONSENT.groups.map((g) => (
                    <div key={g.g} style={{ marginTop: 9, border: "1px solid var(--border)", borderRadius: 9, background: "#fff", padding: "9px 11px" }}>
                      <div style={{ fontSize: 12.2, fontWeight: 800, color: "#12203F" }}>{g.t}</div>
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2, lineHeight: 1.5 }}>{g.why}</div>
                      {g.items.map((it) => (
                        <div key={it.k} style={{ marginTop: 7, borderTop: "1px dashed #EEF2F7", paddingTop: 7 }}>
                          <label style={{ display: "flex", gap: 7, alignItems: "flex-start", cursor: "pointer" }}>
                            <input type="checkbox" style={{ marginTop: 2, width: 17, height: 17, flexShrink: 0 }} checked={!!enroll.ci[it.k]}
                              onChange={(e) => setEnroll((st) => ({ ...st, ci: Object.assign({}, st.ci, { [it.k]: e.target.checked }) }))} />
                            <span style={{ fontSize: 11.8, lineHeight: 1.5 }}>
                              <b>{it.req ? "[필수] " : ""}{it.t}</b>
                              <span style={{ display: "block", color: "var(--muted)", fontSize: 11 }}>{it.why}</span>
                              {it.gain && <span style={{ display: "block", fontSize: 10.8, color: "#15803D", marginTop: 3, lineHeight: 1.5 }}>✅ <b>동의하면</b> — {it.gain}</span>}
                              {it.loss && <span style={{ display: "block", fontSize: 10.8, color: "#B45309", marginTop: 1, lineHeight: 1.5 }}>⚠️ <b>동의하지 않으면</b> — {it.loss}</span>}
                              <button style={{ border: "none", background: "none", color: "#1D4ED8", fontWeight: 700, fontSize: 10.8, padding: 0, marginTop: 2, cursor: "pointer" }}
                                onClick={(e) => { e.preventDefault(); setEnroll((st) => ({ ...st, ciDetail: Object.assign({}, st.ciDetail, { [it.k]: !st.ciDetail[it.k] }) })); }}>
                                {enroll.ciDetail[it.k] ? "내용 접기 ▲" : "내용 보기 ▼"}
                              </button>
                              {enroll.ciDetail[it.k] && <span style={{ display: "block", fontSize: 10.8, color: "#475569", background: "#F8FAFC", borderRadius: 6, padding: "6px 8px", marginTop: 4, lineHeight: 1.55 }}>{it.d}</span>}
                            </span>
                          </label>
                        </div>
                      ))}
                      {g.note && <div style={{ fontSize: 10.6, color: "var(--soft)", marginTop: 7, lineHeight: 1.5, borderTop: "1px dashed #EEF2F7", paddingTop: 6 }}>※ {g.note}</div>}
                    </div>
                  ))}
                  {/* 3층 — 정식 전문 */}
                  {(!ciOk || enroll.ciOpen) && (
                    <details style={{ marginTop: 8, fontSize: 11 }}>
                      <summary style={{ cursor: "pointer", fontWeight: 700, color: "var(--soft)" }}>정식 동의서 전문 보기 — 아래가 법적 기준이며, 위 요약은 이해를 돕기 위한 것입니다</summary>
                      <div style={{ color: "#475569", lineHeight: 1.62, marginTop: 6 }}>
                        <b>1. 수집·이용</b> — 목적: 보험계약의 인수심사·체결·이행·유지·관리, 보험료 산출·검증, 민원처리 및 분쟁 대응, 금융거래 업무, 보험모집질서 유지, 가입한 보험계약 상담, 만족도 조사, 보험금 지급·심사, 모집인 수당 확인, 건강등급 산정, 기존 계약과의 중요사항 비교설명. 보유·이용기간: 동의일부터 거래 종료 후 3년(거래 종료일 = 만기·해지·취소·철회·소멸일, 보험금 청구권 소멸시효 완성일, 채권·채무 소멸일 중 가장 나중에 도래한 날).<br /><br />
                        <b>2. 제공</b> — 제공받는 자: 금융위원회·금융감독원·국세청·보험요율산출기관 등 법정 업무수행기관, 한국신용정보원, 생명·손해보험회사 및 국내 재보험사·공제사업자·체신관서, 계좌개설 금융기관·금융결제원·은행연합회, 생명·손해보험협회. 제공 목적: 법령상 업무 수행, 신용정보 집중관리·활용, 중복보험 확인 및 비례보상, 재보험 가입, 보험계약 공동인수, 의료심사·자문, 사고처리·민원접수 등. 보유기간: 제공받는 자의 이용목적 달성 시까지(관련 법령상 보존기간에 따름). ※ 업무위탁 목적으로 처리하는 경우 별도 동의 없이 수탁자에게 제공될 수 있습니다.<br /><br />
                        <b>3. 조회</b> — 조회 대상: 한국신용정보원, 생명·손해보험협회, 보험요율산출기관 등. 조회 목적: 보험계약의 인수심사·체결·이행·유지·관리(부활 및 갱신 포함), 가입한도 조회, 실손형 보험 중복 확인, 기존 계약과의 중요사항 비교설명, 세금우대저축사항 확인, 보험금 지급·심사, 보험사고 조사. 조회 결과 보험거래가 개시되면 거래 종료 후 3년까지 동의 효력이 지속되며, 거래가 거절된 경우 그 시점부터 효력이 소멸합니다.<br /><br />
                        <b>공통</b> — 동의를 거부하실 수 있으며, 신용도 평가 목적 외의 제공 동의는 철회할 수 있습니다. 다만 본 동의는 보험계약 인수심사·체결·이행·유지·관리에 필수적이므로 거부 시 해당 업무 수행이 불가능합니다. 본 동의문은 초안이며 시행 전 법무 검토가 필요합니다.
                      </div>
                    </details>
                  )}
                </div>
                <label className={"bkinsagree" + (enroll.mkt ? " on" : "")} style={{ marginTop: 5, alignItems: "flex-start" }}>
                  ...
                  <span className="ba-t">③ [필수] 보험계약 상담·안내에 동의합니다
                    <span style={{ display: "block", color: "var(--muted)", fontStyle: "normal", fontSize: 11, lineHeight: 1.6, marginTop: 3 }}>
                      ...
                      · 서비스 만족도 조사 · 보장 만기 시 재가입·맞춤 상품 안내
                    </span>
                    <i>— 보험업법에 따른 계약 관리·중요사항 비교설명과 보장 만기 안내를 위해 필요해요</i></span>
                </label>
                {/* ④ 보험계약 가입란 — 동의 완료 후에만 노출(부담 완화). 계약자명·주민번호 + 자필서명으로 청약 완결.
                    주민번호: 뒤 7자리 마스킹 입력·평문 미저장(해시만 금고 보관) / 서명: 금고·증서에만 보관 */}
                {enroll.termsOk && enroll.req && ciOk && enroll.mkt && (
                  <div style={{ marginTop: 8, border: "1px solid var(--border)", borderRadius: 10, background: "#fff", padding: "10px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                      <ScrollText size={14} color="#1D4ED8" />
                      <div style={{ fontSize: 12.8, fontWeight: 900 }}>④ 보험계약 가입</div>
                      <span style={{ fontSize: 10.5, fontWeight: 800, color: "#1D4ED8", background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 999, padding: "1px 7px" }}>청약서</span>
                    </div>
                    <div style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.55, marginBottom: 8 }}>보험 계약 성립에는 법령상 성명·주민등록번호 확인이 필요해요. <b style={{ color: "#16A34A" }}>하이핀은 주민등록번호를 보관하지 않아요</b> — 형식 확인 후 검증용 해시만 남기고 평문은 즉시 폐기하며, 화면·기록에는 마스킹(앞 1자리만)으로 표시돼요. 다만 <b>보험업법에 따라 인수사(현대해상)에는 계약 체결을 위해 전달</b>되며, 이는 위 ②-2 동의 항목에 포함돼요.</div>
                    ...
                    {/* 보험계약 가입 서명란 — 자필서명으로 청약 의사표시 완결 */}
                    <div style={{ marginTop: 11, paddingTop: 10, borderTop: "1px dashed var(--border)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 5 }}>
                        <div style={{ fontSize: 12.3, fontWeight: 900 }}>보험계약 가입 서명란</div>
                        <span style={{ fontSize: 10.5, fontWeight: 800, color: "#B42318" }}>필수</span>
                      </div>
                      <div style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.55, marginBottom: 7 }}>
                        위 <b>약관 중요사항 · 가입(청약) · 개인(신용)정보 · 상담·안내</b> 내용을 모두 확인했으며, 계약자 본인이 검진대비보험 가입을 신청합니다. 아래에 자필로 서명해 주세요.
                      </div>
                      <SignPad value={enroll.sign} onChange={(v) => setEnroll((s) => ({ ...s, sign: v }))} />
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 7, fontSize: 11.5, color: "var(--muted)" }}>
                        <span>계약자(피보험자) <b style={{ color: "var(--ink)" }}>{enroll.name.trim() || "—"}</b> (인)</span>
                        <span>신청일 {(() => { const d = new Date(); return `${d.getFullYear()}. ${d.getMonth() + 1}. ${d.getDate()}.`; })()}</span>
                      </div>
                      <div style={{ fontSize: 10.5, color: "var(--muted)", lineHeight: 1.55, marginTop: 6 }}>※ 서명 이미지는 계약 성립의 증빙으로 <b>내 데이터 금고와 가입증서에만</b> 보관되고, 블록체인에는 위변조 확인용 해시만 기록돼요.</div>
                    </div>
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 8, marginTop: 9 }}>
                  <button className="cbtn pri" style={{ margin: 0, minWidth: 0, opacity: enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk && signOk ? 1 : .5 }} disabled={!(enroll.termsOk && enroll.req && ciOk && enroll.mkt && idOk && signOk)} onClick={enrollConfirm}><ShieldCheck size={14} /> {!ciOk ? "개인(신용)정보 동의가 필요해요" : (!enroll.mkt ? "상담·안내 동의가 필요해요" : (!idOk ? "계약자명·주민번호를 입력하세요" : (!signOk ? "서명란에 자필서명이 필요해요" : "가입 확정 · 보험료 0원")))}</button>
            ...
            <div className="bkconfirm">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 16, textAlign: "left" }}>
                {insAgree ? (<>
                  ...
                  <div className="resitem" style={{ margin: 0 }}><span className="ic"><Stethoscope size={18} color="#1D4ED8" /></span><div><b style={{ fontSize: 13 }}>검진 후 건강상담까지 이어져요</b><div style={{ fontSize: 11.5, color: "var(--muted)" }}>결과가 나오면 우리 동네 전문의 원격 상담으로 — 원격진료·건강상담 탭에서 기다릴게요.</div><button className="cbtn" style={{ marginTop: 7, padding: "7px 13px", fontSize: 12 }} onClick={() => { onClose(); try { if (_checkupGo) _checkupGo("tele"); } catch (e) {} }}><Video size={13} /> 원격진료·건강상담 미리 보기</button></div></div>
      ...
      <div style={{ width: "min(560px, 96vw)", maxHeight: "92vh", overflowY: "auto", background: "#fff", borderRadius: 18, boxShadow: "0 30px 70px -30px rgba(0,0,0,.5)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: "16px 22px 20px" }}>
          ...
          {cert.insured && cert.insured.sign && (
            <div style={{ marginTop: 12, border: "1px solid #E2E8F0", borderRadius: 11, padding: "9px 13px 7px", background: "#FCFDFF" }}>
              <div style={{ fontSize: 11.5, fontWeight: 800, color: "#64748B" }}>계약자 자필서명</div>
              <img src={cert.insured.sign} alt="계약자 자필서명" style={{ display: "block", width: "100%", maxWidth: 300, height: "auto", margin: "2px 0 0" }} />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748B", borderTop: "1px solid #EEF2F7", paddingTop: 5, marginTop: 2 }}>
                <span>{cert.insured.name} (인)</span>
                <span>{cert.insured.signedAt ? (() => { const d = new Date(cert.insured.signedAt); return `${d.getFullYear()}. ${d.getMonth() + 1}. ${d.getDate()}. 서명`; })() : ""}</span>
              </div>
            </div>
          )}
          {/* 블록체인 검증 */}
...
/* ── 전국 검진기관 데이터셋(data/checkup_orgs.json — 공단 검진기관기본 × 심평원 좌표 조인) ──
   빌드: scripts/build_checkup_orgs.py · 번들 밖 fetch(hira 패턴) · 실패 시 큐레이션만으로 동작(폴백) */
let _orgsPromise = null;
function loadCheckupOrgs() {
  if (!_orgsPromise) {
    _orgsPromise = fetch("./data/checkup_orgs.json").then((r) => {
      if (!r.ok) throw new Error("전국 검진기관 데이터 로드 실패 (" + r.status + ")");
      return r.json();
    });
  }
  return _orgsPromise;
}
function useCheckupOrgs() {
  const [state, setState] = useState({ loading: true, error: null, data: null });
  useEffect(() => {
    let on = true;
    loadCheckupOrgs().then((d) => on && setState({ loading: false, error: null, data: d }))
      .catch((e) => on && setState({ loading: false, error: e.message, data: null }));
    return () => { on = false; };
  }, []);
  return state;
