  ...
  groups: [
    ...
    { g: "give", t: "② 심사·지급에 필요한 곳에 전달합니다", why: "중복 확인·재보험·의료 심사처럼 법이 정한 업무에 쓰여요.",
      items: [
        ...
        { k: "ci_give_extra", req: false, t: "[선택] 성별·연령·보험계약 종류 추가 제공", why: "상품 개발·통계에 쓰여요. 동의하지 않아도 가입에 불이익이 없어요.", d: "성별, 연령, 보험계약정보(상품종류·기간)" },
      ] },
    ...
    { g: "care", t: "④ [선택] 건강 유지·증진 및 질병의 사전예방·악화 방지를 위한 건강관리 서비스에 씁니다",
      ...
      items: [
        { k: "ci_care_svc", req: false, t: "[선택] 건강관리 서비스 제공(결과 해설·코칭·추적 관리)", why: "이 동의가 없으면 담당 프로의 결과 해설·코칭·추적 관리를 받으실 수 없어요 — 동의는 언제든 철회할 수 있어요.", d: "건강검진 결과의 등급·관리필요 항목(원본 수치 아님), 건강관리 활동 기록(진료 안내·미션·기기 기록), 연락 가능 시간대" },
      ] },
    ...
    { g: "matan", t: "⑤ [선택] 보장이 끝나기 전에 내 보장 상태를 분석해 드립니다",
      ...
      items: [
        { k: "ci_matan_run", req: false, t: "[선택] 만기 시 보장분석 및 안내", why: "만기 20일 전 자동 분석 → 보장맵 안내. 분석에는 이미 가입된 계약 정보만 쓰여요.", d: "보유 보험계약 정보(상품종류·보장항목·한도·기간), 검진대비보험 만기일 — 건강정보 미포함" },
        { k: "ci_matan_id", req: false, t: "[선택] 분석을 위한 개인식별번호 이용", why: "내 계약을 정확히 찾기 위해서만 쓰이고, 값은 암호화 저장소에서 꺼내 메모리에서만 사용돼요 — 화면에 표시되지 않아요.", d: "주민등록번호 등 개인식별번호(조회 목적 한정·실행 기록에 식별값 미기록)" },
      ] },
    ...
    { g: "ads", t: "⑥ [선택] 유용한 소식을 보내드려도 될까요 — 받고 싶은 방법만 골라 주세요",
      ...
      items: [
        { k: "ci_ads_push", req: false, t: "[선택] 앱 알림으로 받기", why: "가장 빠르고 조용한 방법이에요.", d: "광고성 앱 푸시 알림" },
        { k: "ci_ads_sms", req: false, t: "[선택] 문자로 받기", why: "앱을 자주 안 여시면 문자가 편해요.", d: "광고성 문자(SMS·알림톡)" },
        { k: "ci_ads_email", req: false, t: "[선택] 이메일로 받기", why: "자세한 소식은 이메일이 좋아요.", d: "광고성 이메일" },
      ] },
  ],
};
            ...
            <div style={{ border: "1.5px solid " + (enroll.st === "done" ? "#16A34A" : enroll.st === "skip" ? "var(--border)" : "#F59E0B"), borderRadius: 13, padding: "12px 13px", background: enroll.st === "done" ? "#F0FDF4" : "#FFFDF7", marginTop: 10 }}>
              {enroll.st === "done" ? (
                ...
                <div style={{ marginTop: 7, border: "1.5px solid " + (ciOk ? "#BBF7D0" : "#E2E8F0"), borderRadius: 11, background: ciOk ? "#F0FDF4" : "#fff", padding: "11px 12px", opacity: enroll.termsOk ? 1 : .55, pointerEvents: enroll.termsOk ? "auto" : "none" }}>
                  ...
                  {!ciOk && !enroll.ciOpen && (
                    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 7, marginTop: 9 }}>
                      <button className="cbtn pri" style={{ margin: 0, minWidth: 0, padding: "9px 10px", fontSize: 12 }}
                        onClick={() => { const c = {}; CI_REQ.forEach((k) => { c[k] = true; }); setEnroll((st) => ({ ...st, ci: Object.assign({}, st.ci, c), ciFull: false })); }}>
                        <Check size={13} /> 필수 항목 한 번에 동의
                      </button>
                      <button className="cbtn" style={{ margin: 0, minWidth: 0, padding: "9px 10px", fontSize: 12 }}
                        onClick={() => setEnroll((st) => ({ ...st, ciOpen: true, ciFull: true }))}>항목별로 확인</button>
                    </div>
                  )}
                  {(ciOk || enroll.ciOpen) && (
                    <button className="cbtn" style={{ margin: "8px 0 0", padding: "6px 11px", fontSize: 11.5 }}
                      onClick={() => setEnroll((st) => ({ ...st, ciOpen: !st.ciOpen }))}>{enroll.ciOpen ? "항목 접기" : "동의한 항목 보기 · 변경"}</button>
                  )}
                  {/* 2층 — 3그룹 × 개별 체크(원문 9건) */}
                  {enroll.ciOpen && CI_CONSENT.groups.map((g) => (
