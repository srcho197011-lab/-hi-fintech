...
function ConsultIntel({ onGo }) {
  ...
  const [selId, setSelId] = useState((cur && (cur.id || cur.email)) || (roster[0] && (roster[0].id || roster[0].email)) || "");
  const member = roster.find((m) => (m.id || m.email) === selId) || roster[0];
  const A = React.useMemo(() => (member && typeof analyzeConsults === "function" ? analyzeConsults(member) : null), [selId, member]);
