...
function _buildMemberResource(m) {
  ...
  const uniqueKeys = new Set(items.map((r) => r.key)).size;
  const unitCount = items.filter((r) => r.unit).length;
  const refCount = items.filter((r) => r.ref).length;
  const seriesOk = items.every((r) => r.series && r.series.length === 3 && r.series.every((p) => typeof p.value === "number" && !isNaN(p.value)));
  const examYear = chk.years ? chk.years[chk.years.length - 1] : null;
  const checksum = "sha·" + _dconHash(JSON.stringify(items.map((r) => [r.key, r.value]))).toString(16);
  return {
    name: m.name, id: m.id || m.email, judgment: chk.nat.grade, itemCount: items.length, abnormals: chk.comp.abnormals.length,
    trend: chk.trendLabel, bioAge: R ? R.bio : m.biologicalAge, cancerGrade: R ? R.cancerTotal : m.cancerRiskGrade, cost: R ? R.costThis : m.estimatedMedicalCost, recordCount,
    org: "국민건강보험공단", source: chk.comp && chk.comp.src, modelVersion: "checkup-engine v1.0", examYear, uniqueKeys, unitCount, refCount, seriesOk, checksum,
  };
}
/* 데이터 품질검증 11항목(§6.3) — 미검증 데이터는 AI 확정 상담근거로 미사용 */
function _verifyResource(m, res) {
  const checks = [
    ["회원 신원 일치", !!(res && res.name === m.name)],
    ["데이터 생성기관 확인", !!(res && res.org)],
    ["검사일(연도) 유효", !!(res && res.examYear)],
    ["데이터 누락 없음", !!(res && res.itemCount >= 15)],
    ["중복값 없음", !!(res && res.uniqueKeys === res.itemCount)],
    ["단위 표기 정상", !!(res && res.unitCount === res.itemCount)],
    ["기준범위(참고치) 존재", !!(res && res.refCount === res.itemCount)],
    ["과거 데이터 충돌 없음(3년 일관)", !!(res && res.seriesOk)],
    ["분석 모델버전 명시", !!(res && res.modelVersion)],
    ["원천데이터 확인", !!(res && res.source)],
    ["전송 무결성(체크섬)", !!(res && res.checksum)],
  ];
...
function DconMemberSync({ nhisOn, onList }) {
  ...
  return (
    <div className="dcms">
      ...
      <div className="chnote" style={{ marginTop: 10 }}>※ 회원 실데이터는 커넥터(국민건강보험공단 MCP 등)를 통해 수신한 검진 레코드로, <b>genMemberCheckup 엔진의 실구조 데이터</b>를 사용합니다. 각 연동 건은 사업방법서 §6.3의 <b>데이터 품질검증 11항목</b>(신원·생성기관·검사일·누락·중복·단위·기준범위·과거충돌·모델버전·원천·무결성)으로 자동 검증되며, <b>검증 미통과 데이터는 AI 확정 상담근거로 사용하지 않습니다.</b> 실서비스에서는 본인 동의(마이데이터·DID) 하에 각 기관 인증을 거쳐 수신합니다.</div>
    </div>
  );
}
