function DemoLoginSelector({ onLogin }) {
  ...
  const quick = () => { const m = list.find((x) => x.id === selId); if (!m) { setErr("데모 회원을 선택하세요."); return; } tryLogin(m.email, "Demo@1234"); };
  return (
    <div className="card demologin">
      <div className="rct"><CircleUserRound size={18} color="#2563EB" /> 데모 회원으로 로그인 <span className="demobadge"><AlertTriangle size={12} /> 시연용 데모 데이터입니다.</span></div>
      <div className="cfield"><label>데모 회원 선택</label>
      ...
      <button className="cbtn pri" style={{ marginTop: 10 }} onClick={quick}><CircleUserRound size={15} /> 선택한 데모 회원으로 로그인</button>
      ...
      <div className="chnote">※ 공통 비밀번호: Demo@1234 · 시연용 데모 데이터이며 실제 개인정보가 아닙니다.</div>
    </div>
  );
}
