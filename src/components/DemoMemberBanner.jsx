...
function DemoMemberBanner() {
  ...
  let src = null; try { const v = (typeof vaultLoad === "function" && typeof anonToken === "function") ? vaultLoad(anonToken(m)) : null; const ck = v && v.checkups && v.checkups[v.checkups.length - 1]; if (ck) { const y = (ck.date || "2025").slice(0, 4); src = (ck.channel === "nhis" ? `내 ${y}년 국가검진(공단연계·부분)` : ck.channel === "photo" ? `내 ${y}년 검진결과(촬영)` : `내 ${y}년 국가검진 기준`); } } catch (e) {}
  ...
  return (
    <div className="demoband">
      <div className="dbh"><span className="pa">{m.name[0]}</span>
        ...
        {src ? <span className="dbsrc"><ShieldCheck size={12} /> {src}</span> : <span className="demobadge"><AlertTriangle size={12} /> 시연용 예시 데이터</span>}</div>
