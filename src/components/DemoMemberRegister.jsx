...
function DemoMemberRegister({ onLogin, onGoLogin, onChange }) {
  ...
  const doRegister = () => { const r = demoRegisterAll(); setTick((t) => t + 1); onChange && onChange(); setMsg(r.added === 0 ? "이미 등록된 체험 회원은 제외하고 신규 회원만 등록했습니다." : (r.skipped > 0 ? `이미 등록된 체험 회원은 제외하고 신규 ${r.added}명만 등록했습니다.` : `체험 회원 ${r.added}명이 등록되었습니다.`)); };
  const exportDemo = (kind) => {
    ...
    try { const b = new Blob([content], { type: mime }); const url = URL.createObjectURL(b); const a = document.createElement("a"); a.href = url; a.download = `demo_members.${ext}`; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(() => URL.revokeObjectURL(url), 1500); toast(`체험 회원을 ${ext.toUpperCase()}로 내보냈습니다.`); } catch (e) { toast("내보내기에 실패했습니다."); }
  };
  ...
  return (<>
    <div className="card">
      <div className="rct"><Users size={18} color="#2563EB" /> 체험 회원 일괄 등록 <span style={{ marginLeft: "auto", fontSize: 12, fontWeight: 700, color: "var(--muted)" }}>등록됨 {registered.length} / 전체 {(demoMembers || []).length}명</span></div>
      <button className="cbtn pri" onClick={doRegister}><Check size={15} /> 체험 회원 16명 등록하기</button>
    ...
    <div className="card">
      <div className="rct"><ClipboardList size={18} color="#7C3AED" /> 등록된 체험 회원 목록</div>
