...
function FamilyCareSection({ member, onGo }) {
  ...
  const addSpendItem = (opt) => { const e = { id: "s" + Date.now(), item: opt.item, amount: opt.amount }; const next = [...spend, e]; setSpend(next); lsSave("hifin_famspend_" + email, next); if (typeof txAnchor === "function") txAnchor({ kind: "가족지갑 결제", amount: opt.amount, memo: opt.item }); if (typeof toast === "function") toast(`💳 가족지갑에서 ${opt.item} ${opt.amount.toLocaleString()} HTK 결제`); };
