...
const FIN_SGA_META = [["payroll", "인건비", "#F59E0B"], ["marketing", "마케팅비", "#EC4899"], ["rnd", "연구개발비", "#8B5CF6"], ["rent", "영업·관리비", "#64748B"], ["depr", "감가상각비", "#0EA5E9"], ["reward", "포인트(토큰적립) 비용", "#22D3EE"], ["donation", "기부금(치료비 나눔)", "#E11D48"]];
const FIN_LIAB_META = [["token", "토큰적립금 (계약부채)", "#22D3EE"], ["donation", "기부금 준비금 (미지급기부금)", "#E11D48"], ["payable", "매입채무·미지급금", "#94A3B8"]];
/* ══ 시뮬 보정 — 재무모델에서 유도한다(상수 하드코딩 금지) ══
   [문제] 전에는 FIN_FIX가 매 틱 **절대값**으로 쌓였다. 틱당 고정비 약 68만원 vs 틱당 기대매출 약 84만원
   → 변동비를 빼면 남을 수 없어 **구조적으로 영업적자**가 났다(화면 상단에 영업이익 −1,014만원).
   재무모델의 1차연도는 +1.2%인데 화면은 적자를 띄우고 있었으니, 같은 회사의 두 숫자가 어긋난 셈이다.
   [해법] 매출 구성비와 고정비율을 finYears()의 해당 연차에서 그대로 가져온다.
   그러면 시뮬 이익률이 계획 이익률로 수렴하고, 모델을 고치면 화면이 자동으로 따라온다. */
function finSimSpec(y) {
  let r = null;
  try { const rows = (typeof finYears === "function") ? finYears(5) : null; r = rows && rows[y || 0]; } catch (e) { r = null; }
  if (!r || !r.revenue) return null;
  const rv = r.revenue;
  return {
    y: y || 0, label: r.label, opMargin: r.opMargin,
    /* 매출 구성비 — 어떤 스트림이 얼마나 자주/크게 발생할지의 근거 */
    share: {
      product: r.revProduct / rv, checkup: r.revCheckup / rv, service: r.revService / rv,
      reservation: r.revReservation / rv, emr: r.revSub / rv, insurance: r.revInsurance / rv,
      ad: (r.revAd + r.revAgent + r.revApi) / rv,
    },
    /* 건당 단가 — 모델 파라미터 그대로. 구독은 그 연차의 월 구독료(고정)라 범위를 두지 않는다.
       전에는 20만~350만 범위라 표본이 적을 때 실현 구성비가 크게 흔들렸다(5차 이익률 6.6%p 이탈). */
    unit: {
      checkup: [25000, 25000],
      insurance: [70000, 70000],
      reservation: [10000, 10000],
      service: [15000, 15000],
      emr: r.subFee ? [r.subFee, r.subFee] : [0, 0],
      product: [20000, 150000],
      ad: [30000, 120000],
    },
    /* 고정 판관비 — 매출 대비 비율(적립·기부는 제품마진에서 따로 계산되므로 제외) */
    fixRate: {
      payroll: r.payroll / rv,
      marketing: r.marketing / rv,                       // CAC + 브랜드·퍼포먼스
      rnd: (r.rnd + r.cloud + r.gpu) / rv,               // R&D·클라우드·GPU
      rent: (r.salesCost + r.adminCost) / rv,            // 영업·관리비
      depr: 0.002,                                        // 감가상각(연 10억 규모를 매출비로 근사)
    },
  };
}
const FIN_FIX_FALLBACK = { payroll: 0.158, marketing: 0.117, rnd: 0.010, rent: 0.018, depr: 0.002 };
...
/* 스트림 선택 — 계획의 매출 구성비를 재현한다.
   한 건당 평균 금액이 스트림마다 다르므로(구독 185만 vs 예약 1만), 발생 확률은 목표비중÷평균단가에 비례해야
   금액 기준 비중이 계획과 같아진다. spec이 없으면 기존 고정 가중치로 폴백. */
const _finWpick = (spec) => {
  const sh = spec && spec.share;
  const ws = FIN_REVTYPES.map((x) => {
    if (!sh) return x.w;
    const target = sh[x.k] || 0;
    const u = (spec.unit && spec.unit[x.k]) || [x.min, x.max];
    const mean = (u[0] + u[1]) / 2 || 1;
    return target / mean;
  });
  const t = ws.reduce((s, v) => s + v, 0);
  if (!t) return FIN_REVTYPES[0];
  let r = Math.random() * t;
  for (let i = 0; i < FIN_REVTYPES.length; i++) { r -= ws[i]; if (r <= 0) return FIN_REVTYPES[i]; }
  return FIN_REVTYPES[0];
};
...
function FinanceLive() {
  ...
  /* 시뮬이 어느 연차의 구조를 재현할지 — 기본 1차연도(계획 첫 해). 형이 시연 중 바꿀 수 있다. */
  const [simYear, setSimYear] = useState(0);
  const simSpec = React.useMemo(() => finSimSpec(simYear), [simYear, pTick]);
  const specRef = useRef(null);
  useEffect(() => { specRef.current = simSpec; }, [simSpec]);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      let tickRev = 0;
      for (let e = 0; e < nEv; e++) {
        const t = _finWpick(spec);
        const u = (spec && spec.unit && spec.unit[t.k]) || [t.min, t.max];
        const p = Math.round(finRr(u[0], u[1]) / 1000) * 1000; const who = pickWho(t.k);
        a.rev[t.k] += p; tickRev += p;
        if (t.k === "product") {
      ...
      /* 고정 판관비 — 그 틱 매출에 계획 비율을 곱해 인식한다(±8% 흔들림만 준다).
         절대값으로 쌓던 때는 매출과 무관해 구조적 적자가 났다(finSimSpec 주석 참조). */
      const fr = (spec && spec.fixRate) || FIN_FIX_FALLBACK;
      for (const k in fr) a.sga[k] += Math.round(tickRev * fr[k] * (0.92 + Math.random() * 0.16));

      // 금융비용(대출·리스 이자) / 금융수익(예금이자)
      a.other.finCost += Math.round(tickRev * 0.0018);
      if (Math.random() < 0.5) a.other.finIncome += Math.round(tickRev * 0.0004);
  ...
  return (<>
    <div className="ontsimbar">
      <div className={`ontsimstate ${running ? "on" : ""}`}><span className="dot" /> {running ? "회계기간 진행 중" : "일시정지"} <em>· 거래 {tick.toLocaleString()}건 · {simSpec ? simSpec.label : "1차연도"} 계획구조 재현</em></div>
      <div className="ontsimctl" style={{ marginLeft: "auto" }}>
        <span style={{ fontSize: 11.5, color: "var(--soft)", marginRight: 4 }}>재현 연차</span>
        {[0, 1, 2, 3, 4].map((yy) => <button key={yy} className={simYear === yy ? "on" : ""} onClick={() => { setSimYear(yy); reset(); }}>{yy + 1}차</button>)}
      </div>
    ...
    {/* KPI는 「거래 N건을 분개한 누적치」다 — 연간 실적이 아니다.
        전에는 라벨이 그냥 「매출액·영업이익」이라 회사 실적으로 읽혔고, 시뮬 특성상 적자가 떠 있었다. */}
    <div className="ontkpis" style={{ gridTemplateColumns: "repeat(5,1fr)" }}>
      {[["매출액", finWon(pl.revenue), "#22D3EE", "분개 누적"],
        ["영업이익", finWon(pl.op), pl.op >= 0 ? "#34D399" : "#EF4444", pl.revenue ? `이익률 ${(pl.op / pl.revenue * 100).toFixed(1)}%` : "분개 누적"],
        ["당기순이익", finWon(pl.net), pl.net >= 0 ? "#34D399" : "#EF4444", "분개 누적"],
        ["자산 총계", finWon(bs.assets), "#FBBF24", "누적 기준"],
        ["부채비율", (bs.debtRatio * 100).toFixed(0) + "%", "#A78BFA", "부채÷자본"]].map(([k, v, c, sub], i) => (
    ...
    {simSpec && (
      <div className="finlink" style={{ background: "#0C2A20", borderColor: "#1F5137" }}>
        <Check size={13} color="#34D399" /> 위 수치는 <b>거래 {tick.toLocaleString()}건을 분개한 누적치</b>이지 연간 실적이 아닙니다 —
        {simSpec.label} <b>계획 구조</b>(매출 구성비·고정비율)를 그대로 재현하므로, 거래가 쌓일수록
        영업이익률이 계획치 <b>{(simSpec.opMargin * 100).toFixed(1)}%</b>로 수렴합니다.
        {simYear === 0 && <> 1차연도는 <b>AI 플랫폼 구독이 무료(시장 선점)</b>라 구독 매출이 발생하지 않습니다.</>}
      </div>)}
    ...
    {tab === "my" && (() => { const my = finMultiYear(); const revMax = Math.max(...my.map((r) => r.revenue)); const M = (l, f, cls) => <tr className={cls || ""}><td className="mono0">{l}</td>{my.map((r, i) => <td key={i} className="mono">{f(r)}</td>)}</tr>;
      /* 산식을 라벨 아래 붙인다 — 「이 숫자가 어디서 나왔나」를 표에서 바로 읽게 한다(IM 대응) */
      const MF = (l, f, formula, cls) => <tr className={cls || ""}><td className="mono0 hasf">{l}<em className="finf">{formula}</em></td>{my.map((r, i) => <td key={i} className="mono">{f(r)}</td>)}</tr>; return (<>
      ...
      <div className="ontpanel">
        ...
        <div className="onttbl-wrap"><table className="onttbl mytbl">
          ...
          <tbody>
            ...
            {MF("　제품판매(GMV)", (r) => finWon(r.revProduct), "구매회원(회원×38%) × 카테고리 ARPU 합 31.0만원 × 지갑 점유율 70%")}
            {MF("　검진 연계 수수료", (r) => finWon(r.revCheckup), "하이핀 경유 검진 예약(연) × 건당 2.5만원")}
            {MF("　헬스케어 서비스 수수료", (r) => finWon(r.revService), "회원 × 이용률 30% × 건당 1.5만원")}
            {MF("　예약 서비스(골프 등)", (r) => finWon(r.revReservation), "활성회원 × 연 예약 0.4→1.6건 × 건당 1.0만원")}
            {MF("　AI 플랫폼 구독(원격진료·UPI·EMR 사용료 등)", (r) => finWon(r.revSub), "제휴 기관 수 × 월 구독료 × 12개월 (1차 무료 → 2차 50만 → 매년 +50만 · 한도 300만)")}
            {MF("　보험 중개", (r) => finWon(r.revInsurance), "마케팅 동의 회원 × 전환 60% × 건당 7.0만원")}
            {MF("　AI Agent·API·데이터", (r) => finWon(r.revAgent + r.revApi), "회원 × 프리미엄 구독률(0→8%) × 연 2.4만원  +  B2B 계약 수 × 연 6,000만원")}
            {MF("　광고·제휴", (r) => finWon(r.revAd), "활성회원 × 1인당 광고단가(0→2,500원)")}
            {M("매출총이익", (r) => finWon(r.gross), "mysub")}
            {M("(-) 판매관리비", (r) => "-" + finWon(r.sga))}
            {MF("　인건비", (r) => "-" + finWon(r.payroll), "계획 인원(1차 ~70명 → 5차 ~2,000명) 기준 · 5차 매출 대비 약 11%", "mysub2")}
            {MF("　마케팅비", (r) => "-" + finWon(r.marketing), "CAC(신규 순증 × 5,000원) + 브랜드·퍼포먼스(매출 × 8%)", "mysub2")}
            <tr className="mysub2"><td className="mono0 tokc hasf">　포인트(토큰)적립<em className="finf">제품 마진 × 50%</em></td>{my.map((r, i) => <td key={i} className="mono tokc">-{finWon(r.reward)}</td>)}</tr>
            <tr className="mysub2"><td className="mono0 donc hasf">　기부금(치료비 나눔)<em className="finf">제품 마진 × 30%</em></td>{my.map((r, i) => <td key={i} className="mono donc">-{finWon(r.donation)}</td>)}</tr>
            {MF("　기타 운영비", (r) => "-" + finWon(r.otherOpex), "R&D 5% · 영업 2% · 관리 4% · 클라우드 3,000원/활성 · GPU 1,200원/활성 → × 운영 스케일 30%(AI 네이티브)", "mysub2")}
