...
/* ── AI 투자시스템 — 성장 투자 단계(여유자금 운용 없음) · 운용 개시 후 적용할 자산배분 ──
   ※ 데모 정책값: 목표 비중(40/30/10/20)과 원칙 속 수치(듀레이션 3~5년·단일종목 20% 상한·손절 -15%·코인 10% 상한)는
     재무 엔진·예산양식에 없는 시연용 정책안이다(형 확인 사항 — 확정 시 파라미터로 이관). */
const INV_CLASSES = [
...
function AIInvestSystem() {
  ...
  return (
    <div>
      ...
      <div className="chnote" style={{ marginTop: 8 }}>※ 투자 자산·평가·리밸런싱은 시연용 예시입니다. 실제 투자 집행·평가·손익 인식은 투자일임·자본시장법 등 규제 검토와 리스크 관리 정책을 전제로 하며, 특정 상품 매수 권유가 아닙니다.</div>
    </div>
  );
}
