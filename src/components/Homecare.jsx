...
/* ── 지방정부 협력 (공용) — 건강검진·재가돌봄 섹션에서 사용 ── */
const GOV_CO = [["12", "협력 시·도"], ["32", "협약 시·군·구"], ["2026.3", "통합돌봄법 본사업"], ["5", "통합돌봄 영역"]];
const GOV_BENCH = [
  ["🇯🇵 일본", "지역포괄케어 · 후기고령자 검진", "시정촌 중심 의료·개호·예방 통합. 후기고령자(75세+) 검진은 광역연합 재원+시군구 위탁, 総合事業으로 주민참여형 예방돌봄.", "#E11D48"],
  ["🇸🇪 스웨덴·덴마크", "코뮌 중심 재택돌봄", "기초지자체(코뮌)가 고령자 돌봄 정책·실행 전담. ‘자택 거주 지속(aging in place)’ 철학, 저렴한 본인부담.", "#2563EB"],
  ["🇩🇪 독일", "개호보험(Pflege)", "요개호등급 1~5단계 차등급여. 재택 현물·가족돌봄 현금급여, 지역 돌봄거점 상담.", "#0F766E"],
  ["🇬🇧 영국", "통합돌봄체계(ICS)", "NHS(의료)+지방정부(사회적 돌봄) 통합. Care Act 2014 욕구사정, ICS로 의료·돌봄 연계.", "#7C3AED"],
];
const GOV_PROG = {
  checkup: {
    c: "#2563EB", num: "①", title: "어르신 건강검진 서비스 혜택 지원",
    target: "관내 65세 이상 (우선: 국가검진 미수검자·독거·취약계층·후기고령자 75세+)",
    benefit: ["국가건강검진에 더하는 추가 검진·상담 바우처(치매선별·구강·골다공증·우울선별)", "검진 후 AI 건강위험 분석 리포트 제공", "이상소견 시 지역 의료기관 연계 예약", "검진 참여 인센티브(건강 포인트·적립)"],
    gov: "대상자 명부·홍보, 검진 바우처 재원 분담, 보건소·의료기관 연계, 조례 근거 마련",
    platform: "데이터 기반 대상자 선제 발굴, 예약·리마인드 자동화, AI 판독·건강리포트, 사후 돌봄 연계, 지자체 대시보드(수검률·건강지표)",
    fund: "국가건강검진(건보공단) + 지자체 예산 + 플랫폼 운영비 — ‘광역 재원 + 기초지자체 실행’ 매칭펀드(일본 후기고령자 검진 모델)",
  },
  homecare: {
    c: "#E11D8F", num: "②", title: "재가(在宅) 돌봄 서비스 혜택 지원",
    target: "관내 65세 이상 독거·조손·고령부부, 기능저하·우울·고독사 위험군, 요양등급 외 경계선 고령자",
    benefit: ["방문 건강관리·안전확인(AI 안부전화·IoT 센서)", "재가 서비스 연계(방문요양·방문간호·주야간보호)", "사회참여·예방 프로그램(살롱·여가·평생교육)", "응급 시 의료 연계 + 가족돌봄자 교육·상담"],
    gov: "통합돌봄 지역계획 편입, 대상자 판정·명부, 재가 제공기관 지정, 노인맞춤돌봄·치매안심센터 연계",
    platform: "디지털 지역포괄지원센터(원스톱 허브), IoT·AI 안전 모니터링·이상징후 감지, 대상자-서비스 자동 매칭, 다직종 케어 협업, 지자체 대시보드",
    fund: "통합돌봄 사업비 + 장기요양보험(재가급여) + 노인맞춤돌봄 + 지자체 예산 + 플랫폼 — 일본 개호보험·総合事業 재량예산 구조 참고",
  },
};
function GovPartnership({ mode }) {
  const p = GOV_PROG[mode] || GOV_PROG.checkup;
  return (
    <div className="gov">
      <div className="gov-hd"><span className="gov-ic"><Landmark size={19} color="#fff" /></span>
        <div><b>지방정부 협력 <i className="gov-tag">지역사회 통합돌봄</i></b><p>지자체와 협력해 <b>어르신 건강검진·재가 돌봄 혜택</b>을 지원합니다. <b>「의료·요양 등 지역돌봄 통합지원법」 2026.3 본사업</b> 시행에 맞춰, 플랫폼을 지자체의 <b>디지털 지역계획 인프라 파트너</b>로 연계합니다.</p></div></div>
      <div className="gov-co">{GOV_CO.map(([v, k], i) => <div key={i}><b>{v}</b><span>{k}</span></div>)}</div>
      <div className="gov-prog" style={{ borderTopColor: p.c }}>
        <div className="gov-prog-h"><span className="gov-prog-n" style={{ background: p.c }}>{p.num}</span><b>{p.title}</b></div>
        <div className="gov-grid">
          <div className="gov-cell"><span>대상</span><p>{p.target}</p></div>
          <div className="gov-cell"><span>혜택</span><ul>{p.benefit.map((b, i) => <li key={i}>{b}</li>)}</ul></div>
          <div className="gov-cell"><span>지자체 역할</span><p>{p.gov}</p></div>
          <div className="gov-cell"><span>플랫폼 역할</span><p>{p.platform}</p></div>
          <div className="gov-cell full"><span>재원</span><p>{p.fund}</p></div>
        </div>
      </div>
      <div className="gov-bench-t"><Sparkles size={13} color="#7C3AED" /> 해외 벤치마크 · 복지선진국 사례</div>
      <div className="gov-bench">{GOV_BENCH.map(([c, t, d, col], i) => (
        <div className="gov-b" key={i} style={{ borderLeftColor: col }}><b>{c}</b><em>{t}</em><p>{d}</p></div>
      ))}</div>
      <div className="chnote">※ 협력 지자체 수·프로그램은 설계 목표·예시입니다. 실제 협력·혜택·재원은 지자체 조례·예산과 관련 법령(통합돌봄법·노인복지법·건강검진법 등)·협약에 따라 확정됩니다.</div>
    </div>
  );
}
