function MemberHealthDashboard({ member, onGo, onLogout }) {
  ...
  return (
    <>
      <div className="card">
        <div className="rct"><CircleUserRound size={18} color="#2563EB" /> 회원 기본정보 <span className="demobadge"><AlertTriangle size={12} /> 시연용 데모 데이터입니다.</span></div>
        <div className="demoprofile">
          ...
          <div><div className="pn">{member.name} <span className="dgrade" style={{ color: cg[1], background: cg[2], marginLeft: 6 }}>암위험 {member.cancerRiskGrade}등급 · {cg[0]}</span></div>
            <div className="pmeta">{member.email} · 데모회원 {member.isDemoUser ? "예" : "아니오"} · ID {member.id}</div></div>
      ...
      <div className="card demoai">
        ...
        <div className="chnote" style={{ marginTop: 0 }}>로그인한 데모 회원 데이터를 기준으로 AI가 분석합니다. 아래 질문을 눌러보세요.</div>
      ...
      <div className="demodisc"><AlertTriangle size={14} /> 본 내용은 시연용 데모 데이터 기반 분석이며, 실제 의학적 진단이나 보험 가입 심사를 대체하지 않습니다.</div>
    </>
  );
}
