...
const PI_PARAM_DEFAULT = {
  ...
  PER_MEMBER_RATE: 0.001,          // 1인당 신청 한도 (총발행주식수 대비)
...
/* ── 자가검증(로직 테스트) ── */
function piSelfTest() {
  const base = { NET_INCOME_FORECAST: 4518000000, PER: 35, TOTAL_SHARES: 1000000, ANNUAL_POOL_RATE: 0.10, PER_MEMBER_RATE: 0.001 };
  const r = [];
  ...
  r.push({ n: "1인당 한도(0.1%)", exp: "1,000주", got: piNum(piPerMemberCap(base)), pass: piPerMemberCap(base) === 1000 });
  const t1 = piMaxQty(base, 100000, 0); r.push({ n: "적립금 부족(10만원)→0주", exp: "0주", got: piNum(t1), pass: t1 === 0 });
  const t2 = piMaxQty(base, 999999999999, 0); r.push({ n: "1인 상한 초과→1,000주 캡", exp: "1,000주", got: piNum(t2), pass: t2 === 1000 });
