const SHOP_PARTNERS = {
  ...
  supp: [{
    ...
    name: "조윈", brand: "헬스인슈", sub: "영양제 특별제휴", pharmacyFind: true,
    ...
    chips: ["방수밴드", "습윤밴드", "알콜스왑·소독", "상처관리", "의약외품"],
    stats: [["밴드닥터", "상처케어 브랜드"], ["클린덤", "알콜스왑"], ["전국", "약국 공급"]],
    home: "https://hkmedi.co.kr/store/615448#content_01", q: "메디콥 밴드닥터 클린덤 알콜스왑",
  }],
  device: [{
      ...
      {cartOpen && (
        <div className="pdov" onClick={() => { setCartOpen(false); setOrdered(false); }}><div className="pdbox" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="pdbody">
            {ordered ? (
              <div className="ordok"><span className="ic"><Check size={28} color="#16A34A" /></span><b>주문이 접수되었습니다</b><p>{totalCnt}개 · {shopWon(totalPrice)}<br />건강적립금 <b style={{ color: "#B45309" }}>{shopWon(totalReward)}</b>이 결제 확정 시 건강금융지갑에 적립됩니다.</p><button className="cbtn pri" onClick={() => { shopCartClear(); setCartOpen(false); setOrdered(false); }}>확인</button></div>
            ) : (<>
  ...
  return (
    <div className="prodcard" style={{ cursor: "default" }}>
      ...
      <div className="pinfo">
        ...
        <div className="pprice">{won(p.price)}</div>
    ...
    <div style={{ marginTop: 14 }}>
      ...
      <div className="kt shopai" style={{ maxWidth: 720 }}>
        ...
        <div className="kt-input">
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="예: 눈이 침침해요 / 혈당 관리 / 혈압계 추천" />
