...
function socialImpact() {
  ...
  // 재무회계 모델 연동 — 제품마진 → WALLET_SPLIT 적립60%·나눔15%·운영25%(형 확정 2026-09-17, +특별지원: 어르신 치료비·장애아동 재활). 1차연도(10만 회원, 파일럿 규모) 기준.
  const fs = (typeof finSocial === "function") ? finSocial(0) : { margin: 655500000, earn: 393300000, give: 98325000, ops: 163875000, animal: 8193750, beneficiaries: 386 };
...
function SocValueGraph() {
  const nodes = [
    ...
    { id: "earn", t: `건강적립\n환원 ${WALLET_SPLIT.earn}%`, x: 250, y: 178, c: "#16A34A" },
    { id: "give", t: `치료비나눔\n${WALLET_SPLIT.give}%`, x: 390, y: 178, c: "#E11D48" },
  ...
  return (
    <div className="ontgraph">
      ...
      <div className="ontgraph-note">시민의 <b>참여·소비</b>가 <b style={{ color: "#818CF8" }}>판매마진</b>이 되고 → <b style={{ color: "#16A34A" }}>건강적립({WALLET_SPLIT.earn}%)</b>으로 회원에게 환원, <b style={{ color: "#E11D48" }}>치료비 나눔({WALLET_SPLIT.give}%)</b>·<b style={{ color: "#7C3AED" }}>특별지원(어르신·아동)</b>으로 사각지대에 전달되어 <b style={{ color: "#FB7185" }}>사회적 임팩트</b>로 순환합니다.</div>
    </div>
  );
}
...
function SocLiveCounter({ base }) {
  ...
  const items = [
    ...
    { ic: HeartHandshake, c: "#E11D48", label: "오늘 치료비 나눔", v: Math.round((3120000 + t * 8400) * WALLET_SPLIT.give / WALLET_SPLIT.earn), money: true },   // 환원 대비 나눔 = give÷earn(WALLET_SPLIT)
...
const SOC_CERTS = [
  ...
  ["이익 재투자 구조", `판매마진 ${WALLET_SPLIT.earn + WALLET_SPLIT.give}%(적립${WALLET_SPLIT.earn}+나눔${WALLET_SPLIT.give})를 회원·사각지대에 환원 — 이윤의 사회적 재분배`, "#E11D48", "Recycle"],
...
function SocialSection({ onGo }) {
  ...
  const pillars = [
    ...
    { key: "환원", label: "수익의 환원", score: 91, Ic: HandCoins, color: "#16A34A", metric: socWon(si.earn), sub: `판매마진 ${WALLET_SPLIT.earn}%를 건강적립금으로 회원 환원`, to: "wallet" },
    { key: "나눔", label: "기부·나눔", score: 88, Ic: HeartHandshake, color: "#E11D48", metric: socWon(si.give), sub: `판매마진 ${WALLET_SPLIT.give}% 치료비 사각지대 + 특별지원(어르신·장애아동)`, to: "wallet" },
    ...
    <div style={{ marginTop: 16 }}>
      ...
      {tab === "give" && (<>
        ...
        <div className="ontpanel">
          ...
          <div className="chnote" style={{ marginTop: 10 }}>※ 기부 비율(판매마진 25%×5%)은 확정된 배분 원칙이고, 누적 수치는 재무 모델 추정·제휴기관은 예정입니다. 실제 기부는 제휴 공익재단·전문기관 심사를 거쳐 집행되며, 집행내역을 투명하게 공개하도록 설계합니다.</div>
        </div>
      </>)}
    </div>
  );
