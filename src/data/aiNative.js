...
const HIFIN_LEXICON = [
  ...
  ["무료3종", ["3종서비스", "무료3가지", "세가지무료", "3종세트", "무료세트"]],
  ["환불", ["돈돌려", "반품환불"]],
...
const TOOL_RUN = {
  ...
  wallet(m) { try { const lb = (m && typeof tlSync === "function") ? (() => { try { return tlSync(m); } catch (e2) { return null; } })() : null; const base = (typeof WALLET !== "undefined" ? WALLET.total : 12480); const cp = (m && typeof careplanEarned === "function") ? careplanEarned(m.email) : 0; const sp = (typeof shopHtkPts === "function") ? shopHtkPts(m ? m.email : "default") : 0; const d = (typeof htkDelta === "function") ? htkDelta(m) : 0; const tot = (lb != null) ? lb : Math.max(0, base + cp + sp + d); const ins = Math.floor(tot * 0.3); return { lines: [`지금 총 ${tot.toLocaleString()} HTK(≈${_won(tot * 10)})가 있어요${lb != null ? " — 트랜잭션 원장 합산 기준이에요" : ""}.`, `그중 치료비 케어 전용 ${ins.toLocaleString()} HTK, 일반 ${ (tot - ins).toLocaleString()} HTK — 쇼핑·청약에 쓸 수 있어요.`] }; } catch (e) { return null; } },
  ...
  consentdo(m, text) { try {
    ...
    let wd = 0;
    if (!on && (hit[1] === "insurance" || hit[1] === "health")) { try { wd = (typeof lrWithdrawAll === "function") ? lrWithdrawAll(m) : 0; } catch (e) {} }
    return { lines: [
      `${hit[2]} 동의를 ${on ? "켰어요" : "철회했어요"} — 변경 이력이 블록체인에 기록됐고, 언제든 다시 바꿀 수 있어요.`,
      wd ? `배정돼 있던 지역 상담 ${wd}건도 즉시 회수했어요 — 상담 조직의 접촉이 중지되고, 전달 정보 파기 확인(72시간 내)까지 제가 알려드릴게요.` : null,
    ].filter(Boolean) };
  } catch (e) { return null; } },
  ...
  inscover(m) { try {
    ...
    return { lines: [
      ...
      "보장 내용 전체예요:\n" + table,
      `${age}세 ${sex}성 기준 고급형이면, 일반암·뇌졸중·급성심근경색 진단 확정 시 각 최대 1,000만 원까지 받아요(상피내암·갑상선암 등 기타암 제외). 인수는 현대해상 전속대리점 글로벌예방금융㈜ · 가입 내역은 블록체인에 기록돼요.`,
  ...
  ladder(m) { try {
    ...
    return { lines: [L.note, L.stage === "silson-first" ? "치료비 케어 섹션 › 치료비 준비 진단 탭에서 사다리 1단계(실손)부터 밟아보세요." : "치료비 준비 진단 탭에서 예측 위험 기반 보장을 추천받을 수 있어요."], buttons: ["치료비 준비 진단 열어줘"], nav: "insurance" };
  } catch (e) { return null; } },
  // 나눔 현황(과업4 신설) — SharingPool 실적립 원장
  donate() { try {
    const S = (typeof insService !== "undefined") ? insService.donateStatus() : null; if (!S) return null;
    const sharePct = Math.round(((insService.config && insService.config.SHARE_RATE) || 0.15) * 100);   // 나눔 비율은 INS_CONFIG.SHARE_RATE 단일 정의(WALLET_SPLIT.give와 같은 값) — 문구에 숫자를 박지 않는다
    if (!S.count) return { lines: [`아직 나눔 재원 적립 내역이 없어요 — 보험료 납부나 건강쇼핑이 생기면 순환의 ${sharePct}%가 건별로 쌓이고, 여기서 바로 보여드려요.`] };
    return { lines: [`지금까지 나눔 재원에 ${S.balance.toLocaleString()}원(${S.count}건)이 건별 적립됐어요 — 출처: ${Object.entries(S.bySource).map(([k, v]) => `${k} ${v.toLocaleString()}원`).join(" · ")}.`, `치료비 사각지대 지원에 쓰이는 순환의 ${sharePct}% 몫이에요 — 집행은 제휴 재단 심사 후 투명 공개돼요.`] };
  } catch (e) { return null; } },
  ...
  premiumconsult(m) { try {
    ...
    return { lines: [
      "프리미엄보험은 갱신형 간편보험과 달리 장기보험이에요 — 검진·건강데이터로 설계하기 때문에 건강데이터 활용·보험 연계 동의가 먼저 필요해요.",
...
const AGENT_SEC_GUIDES = [
  { k: "checkup", label: "건강검진 예약", words: ["검진예약", "건강검진", "검진센터", "검진받", "검진신청", "검진잡", "검진하러", "검진받고싶", "검진받으려", "국가검진", "종합검진", "검진현황", "예약현황"], guide: "건강검진 예약 화면을 안내해 드릴게요. 전국 제휴 검진센터를 지역·항목·가격으로 비교해 원하는 날짜에 예약하실 수 있어요. 하이핀을 통해 예약하고 검진을 받으시면 " + free3Short() + "가 함께 제공돼요.", btns: ["내 주변 검진센터 찾아줘", "무료 3종이 뭐예요?", "어떤 검진 받아야 해?"] },
  { k: "tele", label: "비대면 원격진료", words: ["원격진료", "비대면진료", "화상진료", "진료받고싶", "진료보고싶", "아픈데진료", "의사만나", "의사연결", "진료연결", "처방받"], guide: "비대면 원격진료를 열어드릴게요. 시·도 → 시·군·구 → 진료과로 좁혀 우리 동네 전문의를 찾고, 예진부터 처방·약국 수령까지 한 흐름으로 이어져요. 제 검진 데이터를 같이 보고 상담해요.", btns: ["증상으로 진료과 찾기", "지금 연결 가능한 의사"] },
  ...
  { k: "manage", label: "내 건강현황", words: ["건강현황", "건강리포트", "리포트보", "건강상태알", "내건강알", "내건강상태", "생체나이보", "건강분석보"], guide: "내 건강현황 화면에서 생체나이·장기나이·암위험·의료비 예측 리포트를 볼 수 있어요. 검진결과가 연결돼 있으면 실제 내 수치 기준이에요.", btns: ["내 리포트 요약", "검진결과 올리기"] },
  ...
  { k: "shop", label: "건강쇼핑", words: ["건강쇼핑", "쇼핑", "뭘먹어야", "뭐먹어야", "먹어야좋", "영양제사", "영양제보", "식단사", "기기사", "홈케어기기", "쇼핑몰", "상품보", "몰열어"], guide: "건강쇼핑을 열어드릴게요. 건강식단·영양제·홈케어 의료기기를 국내 최저가로 제공하고, 검진 결과에 맞춘 맞춤 추천도 있어요. 구매하면 마진의 60%가 HTK로 적립돼요.", btns: ["내 맞춤 추천 보여줘", "혈압에 좋은 영양제", "적립은 얼마나 돼?"] },
  { k: "insurance", label: "치료비 케어", words: ["보험섹션", "보험화면", "보험보", "보험부족", "보장부족", "보험열어", "보장조회", "보험가입", "치료비화면", "보험쪽", "보험메뉴"], guide: "치료비 케어 화면을 안내해 드릴게요. 보장조회 → AI 보험솔루션(보장공백·본인부담) → 간편가입 → 보험금 청구까지 한 곳에서 돼요. 제 데이터 기준 분석은 말만 하시면 바로 해드려요.", btns: ["보장 공백 분석", "내 실손 몇 세대야?", "보험금 청구 어떻게 해?"] },
  { k: "wallet", label: "건강금융지갑", words: ["건강지갑", "금융지갑", "지갑보", "적립금보", "혜택보여", "지갑열", "적립현황", "적립내역", "온체인", "토큰지갑"], guide: "건강금융지갑을 열어드릴게요. HTK 적립 현황과 치료비 케어 전용 적립(30% 우선), 나눔·기부, 온체인 원장(전송·스왑)까지 볼 수 있어요.", btns: ["적립금 얼마 쌓였어?", "토큰 전송하는 법"] },
  ...
  { k: "onboarding", label: "데이터 연결", words: ["데이터연결", "검진연결", "검진결과업로드", "검진결과분석", "결과분석해", "결과올리", "보험연결", "데이터올리"], guide: "데이터 연결을 열어드릴게요. 검진결과(사진·PDF·공단)와 보험(통합조회)을 연결하면 모든 분석이 내 실제 데이터 기준으로 바뀌어요 — 각 1분이면 돼요.", btns: ["사진으로 올려도 돼요?", "공단 조회는 뭐예요?"] },
  ...
  { k: "social", label: "사회적기업", words: ["사회공헌", "사회환원", "기부활동", "나눔활동"], guide: "사회적기업 화면에서 치료비 사각지대 나눔 등 하이핀의 사회환원 활동을 볼 수 있어요. 회원 소비에서 생긴 판매마진의 15%가 나눔 재원이 돼요.", btns: ["나눔은 어떻게 이뤄져요?"] },
...
const AGENT_NAV_LABEL = { story: "활용 스토리", intro: "회사 소개", home: "회사 소개", trust: "신뢰 센터", checkup: "건강검진 예약", care: "검진 후 케어", insurance: "치료비 케어", mywallet: "나의 건강지갑", partner: "제휴·투자", onboarding: "데이터 연결", ontology: "온톨로지", shop: "건강쇼핑", ai: "AI 주치의 상담", manage: "내 건강현황", hospital: "병원진료 안내", homecare: "재가돌봄", wallet: "건강금융지갑", nft: "Health NFT", mypage: "우리가족건강관리", social: "사회적기업", community: "커뮤니티", healthmate: "헬스메이트 센터" };
...
function agentGreeting() {
  ...
  /* 버튼도 '하고 싶은 활동' 언어로 — 웰컴 카드와 같은 말을 써서 회원이 배우는 표현을 하나로 유지한다 */
  let btns = null;
  try { if (typeof hiQuickChips === "function") btns = hiQuickChips(m); } catch (e) {}
  return { text: parts[0], buttons: (btns && btns.length) ? btns : ["내 건강 봐줘", "보장 공백 분석", "검진결과 올리기"] };
}
