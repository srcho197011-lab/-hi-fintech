...
const HIPRO_CATS = [
  ...
  { cat: "video",  ko: "영상 상담", pats: [/(밤|야간|새벽|늦은\s*시간|시간\s*제한|시간대|몇\s*시|아무\s*때|언제든).*(영상|화상)/, /(영상|화상).*(밤|야간|새벽|늦은\s*시간|시간\s*제한|시간대|몇\s*시|아무\s*때|언제든)/, /영상\s*(상담|통화|연결|요청)/, /(화상|카메라)\s*(상담|통화|켜|꺼)/, /녹화/, /(영상|화상).*(왜|안\s*되|못\s*해|불가)/, /화면\s*공유/, /(함께|같이)\s*보(는|기)/, /개입\s*발행/, /(진료|원격진료)\s*완결/, /영상\s*권장/] },
...
function hiproAnswer(q, ctx) {
  ...
  try {
    ...
    if (cat === "video") {
      ...
      /* 시간대 — 「원칙은 고객이 원할 때」(형 확정 2026-09-04). 부재를 침묵으로 두면 옛 규칙이 그대로 학습되므로 적극적으로 말한다.
         다만 「제한 없음」이 「아무 때나 걸어도 된다」로 읽히면 형 지시의 역이 된다 — 때를 정하는 쪽이 회원임을 함께 말한다. */
      if (/(밤|야간|새벽|늦은\s*시간|시간\s*제한|시간대|몇\s*시|아무\s*때|언제든)/.test(t)) {
        return R("video:hours", "정해진 시간대는 없어요 — 때를 정하는 쪽이 회원님이니까요. 먼저 여쭤보시고 회원님이 지금 좋다고 하시면 그때가 상담 시간이에요. 새벽이든 밤이든 상관없어요. 다만 약속 없이 심야에 먼저 거는 건 원칙이 아니에요 — 요청도 접촉이니까요. 연결은 언제나 회원님이 수락하셔야 시작돼요. 광고성 알림의 야간 미발송(밤 9시~아침 8시)은 그대로예요 — 그건 광고고 이건 회원님이 켜는 상담이라 규칙이 달라요.", ["§0-V7", "vsGateOf", "형 지시 2026-09-04"]);
      }
      if (/왜|안\s*되|못\s*해|불가/.test(t) && typeof VIDEO_SPEC !== "undefined") {
        return R("video:gate", "요청이 막히는 이유는 넷이에요 — ①접촉 락(검진 결과 전) ②접촉 보류(G8) ③「영상 상담 이용」 동의 없음 ④이미 사양하신 뒤(재요청 " + VIDEO_SPEC.reRequestMax + "회 한도). 시간대는 이유가 아니에요 — 정해진 시간대는 없고 회원님이 원하시는 때가 기준이에요. 막히면 버튼이 아예 없고 이유가 배지로 보여요.", ["vsGateOf", "VIDEO_SPEC"]);
      }
      if (typeof VIDEO_SPEC !== "undefined") {
        return R("video:what", "전화·문자·앱알림 옆에 놓인 네 번째 채널이에요. 원칙은 회원님이 하고 싶으실 때예요 — 여쭤보고 좋다고 하시면 그때가 시간이고, 정해진 시간대는 없어요. 프로는 요청만 하고 연결은 회원님이 수락하셔야 시작돼요 — 카메라도 회원님이 켜고 끄시고요. " + VIDEO_SPEC.modes.length + "가지 모드(문자·음성·영상)를 통화 중에 오르내릴 수 있는데, 올리는 건 회원님만 하실 수 있어요. 연결이 나빠지면 시스템이 한 단계 낮춰요.", ["§0-V7", "VIDEO_SPEC", "VS_STATES"]);
      }
    }
