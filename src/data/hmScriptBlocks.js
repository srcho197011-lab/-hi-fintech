/* ══════════════ 회원 스크립트 블록(hmScriptBlocks.js) — 지시서 프롬프트 v1.3 §3-S (P2 초판) ══════════════
   ...
   ⚠️ 원칙:
     ...
     · approved:false 블록이 조립에 걸리면 그 카드는 발행 불가(§S-5 ①) — 초판 전체는 검수 대기 상태로 등재한다.
...
const HM_SCRIPT_BLOCKS = [
  /* ── ① 오프닝(4) ── */
  { id: "op-first", part: "opening", ko: "첫 연결", approved: false,
    t: "{가명}님, 안녕하세요. 하이핀 담당 {프로명}입니다. 검진 관련해서 확인드릴 게 있어 연락드렸어요 — 지금 2분 정도 괜찮으세요?" },
  { id: "op-again", part: "opening", ko: "재접촉", approved: false,
    t: "{가명}님, 하이핀 {프로명}입니다. 지난번에 말씀 나눈 것 이어서 짧게 안부 겸 연락드렸어요 — 잠깐 괜찮으세요?" },
  { id: "op-restart", part: "opening", ko: "정체 재개", approved: false,
    t: "{가명}님, 하이핀 {프로명}입니다. 한동안 챙겨드리지 못해서요 — 요즘 건강 관리는 어떠셨어요?" },
  { id: "op-unlock", part: "opening", ko: "락 해제 결합(결과+안내 1회 통합)", approved: false,
    t: "{가명}님, 하이핀 {프로명}입니다. 검진 결과가 도착해서, 결과 살펴보신 것과 앞으로 관리를 한 번에 정리해 드리려고 연락드렸어요." },
  /* ── ② 본론 — 등급 공통 도입(3) + 지표군(6) ── */
  { id: "co-h", part: "core", ko: "본론 도입 · H", approved: false,
    t: "이번 결과에서 {구간표현}으로 확인됐어요. 진단이 아니라 '의사 선생님 확인이 필요한 구간'이라는 뜻이에요." },
  { id: "co-m", part: "core", ko: "본론 도입 · M", approved: false,
    t: "이번 결과에서 {구간표현}으로 나왔어요. 당장 큰일이라는 게 아니라, 지금부터 관리하면 좋아질 수 있는 구간이에요." },
  { id: "co-l", part: "core", ko: "본론 도입 · L", approved: false,
    t: "수치 자체는 정상 범위인데, 흐름을 보면 지금 잡아두시는 게 좋아 보여요. 그래서 미리 연락드렸어요." },
  { id: "co-bp", part: "core", ko: "지표 보충 · 혈압", approved: false,
    t: "혈압은 하루에도 오르내려서, 병원 한 번 수치보다 꾸준한 기록이 더 정확해요." },
  { id: "co-sugar", part: "core", ko: "지표 보충 · 혈당", approved: false,
    t: "혈당은 식사·체중과 같이 움직여서, 검사 확인과 식단을 같이 보시는 게 좋아요." },
  { id: "co-lipid", part: "core", ko: "지표 보충 · 지질", approved: false,
    t: "콜레스테롤은 증상이 없어서 수치로만 관리가 돼요 — 그래서 확인 시점을 놓치지 않는 게 중요해요." },
  { id: "co-liver", part: "core", ko: "지표 보충 · 간", approved: false,
    t: "간 수치는 최근 음주나 드시는 약·영양제의 영향도 받아요. 그 부분도 같이 봐드릴게요." },
  { id: "co-body", part: "core", ko: "지표 보충 · 체격·근골격", approved: false,
    t: "체중·근육은 한 번에 바꾸는 게 아니라 강도를 맞춰 천천히 가는 게 안전해요." },
  { id: "co-organ", part: "core", ko: "지표 보충 · 신장·혈액 등", approved: false,
    t: "이 항목은 다른 수치와 같이 봐야 정확해서, 확인 검사를 한 번 권해드리는 거예요." },
  /* ── ③ 제안 — 개입 7유형(7) ── */
  { id: "ak-clinic", part: "ask", ko: "제안 · 진료 연결", approved: false,
    t: "지금 연결 가능한 의사 선생님께 먼저 보여드리는 걸 권해요. 제가 보내드리는 알림의 버튼 하나면 예약까지 돼요." },
  { id: "ak-recheck", part: "ask", ko: "제안 · 재검진", approved: false,
    t: "{예약처}에서 확인 검사를 한 번 받아보시면 좋겠어요. 원하시는 날짜로 제가 예약을 도와드릴게요." },
  { id: "ak-diet", part: "ask", ko: "제안 · 식단", approved: false,
    t: "식단은 거창한 게 아니라 한 끼 구성부터예요. {가명}님 수치에 맞춘 구성을 앱에 담아드렸어요 — 한번 보시겠어요?" },
  { id: "ak-supp", part: "ask", ko: "제안 · 영양 보충(상호작용 확인 포함)", approved: false,
    t: "도움이 될 수 있는 영양 성분을 담아드렸어요. 드시는 약이 있으면 겹치지 않는지부터 확인해 드릴게요." },
  { id: "ak-move", part: "ask", ko: "제안 · 운동 미션", approved: false,
    t: "무리한 운동보다, {가명}님께 맞는 강도로 시작하는 미션을 넣어드렸어요. 하루 한 번 체크만 해주시면 제가 흐름을 봐드려요." },
  { id: "ak-habit", part: "ask", ko: "제안 · 습관 미션(절주·복약 등)", approved: false,
    t: "작게 시작하는 게 제일 오래가요. 이번 주는 하나만 정해서 같이 체크해 봐요." },
  { id: "ak-family", part: "ask", ko: "제안 · 가족 케어 연결", approved: false,
    t: "가족분이 앱에서 함께 봐주시면 관리가 훨씬 수월해져요. 동의해 주시면 연결해 드릴게요." },
  /* ── ④ 분기 — 4반응(8: 기본+보충) ── */
  { id: "br-yes", part: "branch", ko: "수락", approved: false,
    t: "잘 생각하셨어요. 지금 바로 알림 보내드릴게요 — 버튼만 눌러주시면 끝나요." },
  { id: "br-yes2", part: "branch", ko: "수락 · 마무리 확인", approved: false,
    t: "완료되면 저한테도 표시가 와요. 잘 안 되면 언제든 다시 연락 주세요." },
  { id: "br-hold", part: "branch", ko: "보류(생각해볼게요)", approved: false,
    t: "네, 천천히 보셔도 돼요. 자료는 앱에 넣어드릴 테니 편하실 때 열어보세요." },
  { id: "br-hold2", part: "branch", ko: "보류 · 다음 약속", approved: false,
    t: "그럼 {다음약속}쯤 제가 한 번만 다시 여쭤볼게요. 부담은 안 드릴게요." },
  { id: "br-no", part: "branch", ko: "거절(괜찮아요) — 재권유 1회 한도", approved: false,
    t: "네, 알겠습니다. 부담 드리려는 건 아니에요. 필요하실 때 하이에게 말씀하시면 언제든 이어서 도와드려요." },
  { id: "br-q-serious", part: "branch", ko: "질문(심각한 거예요?) — 한 문장 원칙", approved: false,
    t: "그건 의사 선생님이 봐주셔야 정확해요 — 그래서 연결을 도와드리려는 거예요." },
  { id: "br-q-ins", part: "branch", ko: "질문(보험 관련) — A2 인계 전용", approved: false,
    t: "보장 쪽은 제가 화면 보면서 같이 확인해 드릴게요 — 잠깐 보장분석 화면으로 넘어가서 볼까요?" },
  { id: "br-q-cost", part: "branch", ko: "질문(비용 있어요?)", approved: false,
    t: "지금 안내드린 건 회원 부담 없이 이용하실 수 있어요. 비용이 생기는 단계에선 먼저 금액부터 보여드려요." },
  /* ── ⑤ 클로징(3) ── */
  { id: "cl-done", part: "closing", ko: "완결 확인형", approved: false,
    t: "예약 확인되면 앱으로 알림 가요. 궁금한 건 하이에게 물어보셔도 되고, 저한테 바로 연락 주셔도 돼요." },
  { id: "cl-promise", part: "closing", ko: "약속형", approved: false,
    t: "그럼 {다음약속}에 제가 먼저 연락드릴게요. 그 전에 필요하시면 언제든지요. 건강히 지내세요." },
  { id: "cl-open", part: "closing", ko: "미완결형(재큐 고지)", approved: false,
    t: "오늘은 여기까지만 할게요. 일주일 뒤에 진행 상황 한 번 봐드릴게요 — 편하게 계세요." },
  /* ── 쉬운말 변형(고령 — 문장당 1정보·한자어 회피)(6) ── */
  { id: "op-first-easy", part: "opening", easy: true, ko: "첫 연결 · 쉬운말", approved: false,
    t: "{가명}님, 안녕하세요. 건강 챙겨드리는 {프로명}이에요. 잠깐 통화 괜찮으세요?" },
  { id: "co-h-easy", part: "core", easy: true, ko: "본론 H · 쉬운말", approved: false,
    t: "이번 검사에서 한 가지가 높게 나왔어요. 병원에서 한 번 봐주시는 게 좋아요." },
  { id: "co-m-easy", part: "core", easy: true, ko: "본론 M · 쉬운말", approved: false,
    t: "조금 높은 항목이 하나 있어요. 지금부터 챙기면 좋아질 수 있어요." },
  { id: "ak-clinic-easy", part: "ask", easy: true, ko: "진료 연결 · 쉬운말", approved: false,
    t: "제가 의사 선생님 연결해 드릴게요. 문자 보내드리면 파란 버튼만 눌러주세요." },
  { id: "br-no-easy", part: "branch", easy: true, ko: "거절 · 쉬운말", approved: false,
    t: "네, 알겠어요. 필요하실 때 언제든 전화 주세요." },
  { id: "cl-done-easy", part: "closing", easy: true, ko: "클로징 · 쉬운말", approved: false,
    t: "예약되면 문자 가요. 궁금하면 저한테 전화 주세요. 건강하세요." },
  /* ── ⑥ 채널 변형 규칙(2) — 문장 창작이 아니라 축약 규칙 ── */
  { id: "ch-notif", part: "channel", ko: "앱알림(2문장)", approved: false,
    t: "[규칙] core 도입 1문장 + ask 1문장으로 축약. 링크 버튼 1개(개입 원탭). 존대 유지·이모지 1개 이하." },
  { id: "ch-sms", part: "channel", ko: "문자(80자·링크 1)", approved: false,
