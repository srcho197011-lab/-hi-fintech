...
const insService = {
  ...
  checkupIns(m) {
    ...
    return { hasCheckup, checkupDate, policy: pol || null, timeline, claims,
      coverage: [
        ["검진에서 암이 발견됐어요", "암 진단금", 1000000, "확정 진단과 동시에 — 치료 시작 비용부터 해결"],
        ["뇌졸중·심근경색 진단을 받았어요", "2대 질환 진단금", 500000, "골든타임 치료에 바로 보태세요"],
        ["수술이 필요하대요", "질병 수술비", 300000, "암·뇌·심장뿐 아니라 대부분 질병의 수술 시"],
        ["정밀검사를 더 받아보래요", "정밀검사 지원", 100000, "복부초음파·조직검사 등 추가 검사비"],
      ],   // 카탈로그(CHECK_COVERS 기본형) 근거 — "만약에" 시나리오 프레임
