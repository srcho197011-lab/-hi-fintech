...
const _DEPT_DZ = {
  ...
  ped: ["소아비만", "아토피피부염", "소아천식", "알레르기비염", "성장지연", "ADHD", "수족구병", "성조숙증", "영유아 발달지연"],
  ortho: ["골다공증", "골절", "퇴행성관절염", "척추질환"],
  derma: ["아토피피부염", "건선", "탈모"],
  ophtha: ["근시", "백내장", "녹내장", "당뇨망막병증"],
  ent: ["알레르기비염", "중이염", "어지럼증"],
...
const _DZ_RULES = {
  ...
  // 소아 전용(상한) / 성인 전용(하한)
  "소아비만": { max: 18 }, "성장지연": { max: 18 }, "소아천식": { max: 18 }, "ADHD": { min: 4, max: 18 },
  "수족구병": { max: 12 }, "성조숙증": { min: 6, max: 13 }, "영유아 발달지연": { max: 7 }, "근시": { min: 5 },
  "알레르기비염": { min: 3 }, "아토피피부염": { min: 0 }, "중이염": { max: 60 },
  "건선": { min: 18 }, "탈모": { min: 18 }, "치주질환": { min: 18 }, "어지럼증": { min: 20 }, "천식": { min: 12 },
};
...
function pilotAgg() {
  ...
  const markAbn = {}; let totalCost = 0, needyN = 0, needyCost = 0, gapN = 0, dzMembers = 0, childN = 0;
  for (const m of c) {
    if (m.isChild) childN++;
