...
function pbPay(m, billId) {
  ...
  // 축5 S1-1: 수납 발생 시 순환 마진의 나눔 몫(INS_CONFIG.SHARE_RATE = WALLET_SPLIT.give)을 나눔 재원에 건별 적립(연출 아님 — SharingPool 원장 tx)
  ...
  if (typeof notifPush === "function") notifPush({ ic: "coin", t: "보험료 수납 완료", d: `${b.product} ${b.ym} · ${b.amount.toLocaleString()}원 — 순환의 ${typeof INS_CONFIG !== "undefined" ? Math.round(INS_CONFIG.SHARE_RATE * 100) : WALLET_SPLIT.give}%가 나눔 재원에 적립됐어요`, target: "insurance" });
