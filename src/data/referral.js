/* ══════════ 회원 추천(리퍼럴) — 6대 획득 채널 ⑥ (Phase 3 · 정책 v2) ══════════
   정책 v2 (2026-07-19 재설계 — 보상 적정성·법규 검토 반영):
   ① 보상: 가입 시 추천인·피추천인 각 100 HTK + 피추천인 '첫 검진 완료' 시 추천인 +300 HTK
      → 1건 최대 500 HTK — 보상 재원·수익성 산정 근거는 내부 재무모델(finModel) 기준(대외 비공개)
   ② 1단계만 인정: 직접 추천만 보상, 하위 추천(2단계 이상) 보상 없음 — 다단계판매 구조 아님(방문판매법 준수)
   ③ 코드 귀속(어트리뷰션): 초대 링크에 심긴 공유자 코드로 가입해야만 적립 — 코드 없는 가입은 보상 없음
   ④ 부정 방지: 실명 본인인증 1인 1계정 · 자기 코드 가입 차단 · 행동 기반 본보상 · 부정 수령 회수 (건수 한도 없음 — 형 지시 2026-07-19)
   ⑤ 가족 등록 포함: 우리가족건강관리에 가족을 등록해도 등록 회원에게 100 HTK(가족 확장 채널 ④와 결합)
   ⑥ HTK는 폐쇄형 포인트(현금 아님·양도 불가·플랫폼 내 사용) — 전자금융/가상자산 규제 비저촉 설계. ⚠️ 수치는 시연용 예시 */
const REF_POLICY = { joinInviter: 100, joinInvitee: 100, action: 300, family: 100, level: 1 };
...
/* 가족 등록 보상 — 우리가족건강관리에 가족을 등록하면 등록 회원에게 100 HTK(채널 ④ 가족 확장) */
function refFamilyAdd(email, label) {
  const s = refState(email); s.family = (s.family || 0) + 1; s.htk += REF_POLICY.family; refSave(email, s);
  try { if (typeof vaultAccessLog === "function") vaultAccessLog("", "member", "가족 등록 보상 적립(" + (label || "가족") + ")"); } catch (e) {}
  return s;
}
