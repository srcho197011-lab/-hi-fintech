...
/* 승인된 관리자(전체보기) 계정 — 이 계정만 로그인 허용. TODO(론칭전 폐기): 환경변수/설정으로 이관.
   ⚠️ 개발 기간 한정 교체: hifin/hifin002 → 하이/하이1(2026-08-27) → hi/hi0500(2026-09-10, 형 지시).
      **배포 전 아래 한 줄을 { id: "hifin", pw: "hifin002" } 로 되돌릴 것.**
      이전 계정은 더 이상 통하지 않는다(교체이므로). 화면 안내문에는 표기하지 않는다. */
const AUTH_ADMIN = { id: "hi", pw: "hi0500" };

/* ── 한글 계정의 '영문 자판 표기'를 파생한다(두벌식) ──
   비밀번호는 마스킹되어 보이므로, 한글 입력기가 꺼진 줄 모르고 치면 원인을 알 수 없다.
   같은 키를 눌렀다면 같은 계정으로 본다 — 계정이 늘어나는 것이 아니라 표기만 둘이다.
   예) "하이1" → ㅎㅏㅇㅣ1 → "gkdl1"
   ※ 현재 계정(hi/hi0500)은 영문이라 이 변환은 그대로 통과한다(무해한 무동작).
      계정을 다시 한글로 바꿀 때를 위해 남겨 둔다. */
